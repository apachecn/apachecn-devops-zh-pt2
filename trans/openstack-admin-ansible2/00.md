...
随着 OpenStack 开始被认为更像是一个主流的云平台，在构建后运行它的挑战已经变得普遍。虽然所有云任务都可以通过应用编程接口或命令行界面工具逐个执行，但这不是处理大型云部署的最佳方式。现在很明显，需要更多的自动化方法来管理 OpenStack。大多数组织都在寻求提高业务敏捷性的方法，并且已经意识到仅仅拥有云是不够的。只有通过某种程度的自动化，才能改进应用程序部署、减少基础架构停机时间并消除日常手动任务。OpenStack 和 Ansible 将帮助任何组织缩小这一差距。凭借开放堆栈必须提供的众多基础设施即服务功能，再加上易于使用的配置管理工具 Ansible，确保了更完整的云实施。
...
**新名词**和**重要词语**以粗体显示。您在屏幕上看到的单词，例如菜单或对话框中的单词，会出现在如下文本中:“通过 ****图像**** 选项卡下的**地平线**仪表板查看它们。”
