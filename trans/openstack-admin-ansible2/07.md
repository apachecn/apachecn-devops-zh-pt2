...
CoreOS 的重点是提供一个默认具有集群感知能力的操作系统，使其非常适合容器技术等平台。Docker 也是尝试容器的明显选择，因为正是它让容器再次流行起来。此外，Docker 还有各种各样的映像，可以按原样进行下拉和部署。对于我们的例子，这里我们将回顾一个非常简单的剧本，它将在 CoreOS 上的容器中部署 ELK 堆栈。
