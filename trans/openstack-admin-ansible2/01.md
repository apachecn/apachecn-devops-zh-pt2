...
这项服务是在第五个 OpenStack 版本中引入的。Keystone 是构建在您的 OpenStack 云中的身份验证和授权组件。它的关键作用是处理用户、租户和所有其他 OpenStack 服务的创建、注册和管理。Keystone 将是第一个安装在开放栈云上的组件。它能够连接到外部目录服务，如 LDAP。Keystone 的另一个关键特性是，它基于**基于角色的访问控制** ( **RBAC** )构建。允许云运营商向云消费者提供对单个服务功能的基于角色的独特访问。
