...
CI设计将作为一个蓝图，指导读者回答如何、为什么以及在哪里实现CI。该设计将涵盖实现端到端 CI 管道所涉及的所有必要步骤。
...
如果你还记得的话，在[第 5 章](05.html)、*分布式构建*的*添加Jenkins奴隶–Docker容器*部分，我们学习了创建Docker图像(`maven-build-slave-0.1`)来创建Docker容器(Jenkins奴隶)。我们将在这里使用相同的 Docker 映像来为我们的 CI 管道生成Jenkins从属代理。
