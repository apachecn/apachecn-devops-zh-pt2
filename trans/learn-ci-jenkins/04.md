...
Jenkins带来了一堆要配置的项目。安装的插件越多，需要配置的就越多。在本章中，我们将只介绍Jenkins中的基本管理任务。我们将在接下来的章节中了解更多关于Jenkins配置的信息，其中我们将尝试向Jenkins添加更多插件，以实现**持续集成** ( **CI** )和**持续交付** ( **CD** )。

# Jenkins插件管理器

Jenkins的大部分力量来自插件。Jenkins插件是安装后增强Jenkins功能的软件。安装在 Jenkins 内部的插件在 Jenkins 作业或 Jenkins 系统配置中显示为参数或可配置项，或者在代码片段生成器下显示为事件(如果它与*声明性管道语法*兼容)。

下面的截图显示了Jenkins系统的配置。这是一个配置 SonarQube 工具(静态代码分析工具)的设置。只有在为 SonarQube 安装了 Jenkins 插件后，相应的配置才可用:

![](img/eea7ac4c-9542-48fa-a09f-76ad94c9e5c2.png)

SonarQube settings inside Jenkins system configuration

Jenkins内部有一个特殊的部分来管理插件。在本节中，我们将学习如何使用Jenkins插件管理器管理插件:

1.  从Jenkins仪表盘点击**管理Jenkins**。
2.  进入管理Jenkins页面后，点击**管理插件**。您也可以使用`<Jenkins URL>/pluginManager`链接访问相同的Jenkins插件管理器页面。
...
**可用**选项卡列出了Jenkins可用的所有插件。Jenkins实例上安装的插件不会在此列出。
...
**已安装**选项卡列出了当前安装在您的Jenkins实例上的所有插件。如下图所示，你可以看到有一个卸载插件和降级插件的选项。
...
在**高级**标签下，就在 **HTTP 代理配置**部分之后，你会看到另一个名为**上传插件**的部分。它为您提供了安装或升级Jenkins插件的工具。

当您的 Jenkins 实例没有互联网访问权限，并且您需要一个新插件或需要升级一个现有插件时，此功能非常有用。想象一个场景，你有一个 Jenkins 实例在局域网内运行，但是不能访问互联网，或者我们应该说 Jenkins 在线插件库。在这种情况下，您将首先从在线 Jenkins 存储库中下载所需的 Jenkins 插件，然后使用可移动介质将其传输到 Jenkins 主服务器。最后，您将使用**上传插件**部分来安装所需的Jenkins插件。
...
7.  当你下载一个插件时，下载它的依赖项(其他Jenkins插件)也很重要。
8.  在安装所需插件之前，必须安装所有依赖项(Jenkins插件)。
9.  将此`.hpi`文件(`logstash.hpi`)复制到您的Jenkins服务器或任何可以访问您的Jenkins仪表板的机器上。

10.  现在，登录到你的Jenkins服务器。从Jenkins仪表盘，导航至**管理Jenkins** | **管理插件** | **高级**。
...
如果有人不小心删除了重要的Jenkins配置，会发生什么？虽然使用我们将在*用户管理*部分看到的严格的用户权限可以避免这种情况，但是想象一下，在Jenkins配置中工作的某个人想要恢复到以前稳定的Jenkins配置。

从我们目前所了解到的情况来看，我们知道整个 Jenkins 配置都存储在 Jenkins 主目录下。是`C:\jenkins` (Windows)、`/var/jenkins_home` (Apache Tomcat)、`/var/lib/jenkins` (Linux)。在下一节中，我们将学习如何使用一个插件，即定期备份插件来备份和恢复Jenkins配置。

# 安装定期备份插件

按照给定的步骤安装定期备份插件:

1.  从Jenkins仪表盘，点击**管理Jenkins** | **管理插件**。
...
1.  从Jenkins仪表盘转到**管理Jenkins** | **定期备份管理器**。
...
5.  根目录`<your Jenkins home directory>`是你的Jenkins主目录。
...
现在让我们测试恢复Jenkins备份。但是在此之前，让我们进行一些配置更改，看看恢复操作是否有效。为此，我们将在“配置系统”页面上进行一些配置更改:

1.  从Jenkins仪表盘，点击**管理Jenkins|配置系统**。
2.  在**配置系统**页面上，更改以下字段的值。
3.  将“执行者数量”字段的值从`2`更改为`5`。
4.  将安静时段字段的值从`5`更改为`10`。
5.  点击页面底部的**保存**按钮。
6.  现在，让我们将Jenkins还原到上述变化之前的某个点。
7.  从Jenkins仪表盘，点击**管理Jenkins|定期备份管理器**。
8.  在结果页面上，选择我们在上一节中创建的备份，并点击**恢复所选备份**按钮。
9.  您将看到以下消息:

正在恢复备份…

10.  刷新页面，从Jenkins仪表盘点击**管理Jenkins|配置系统**。
...
1.  从Jenkins仪表盘，点击**管理Jenkins|系统日志**。
...
Jenkins发布有两种: *LTS 发布*和*周发布*。*Jenkins每周发布版*包含新功能和错误修复，而 *LTS(长期支持)* *发布版*是被认为在 12 周内稳定的特殊发布。建议您始终为Jenkins服务器选择一个 *LTS 版本*:
...
3.  在我们升级Jenkins之前，备份我们的`jenkins_home`目录是很重要的。请参考*创建Jenkins备份*部分。

在升级Jenkins之前，请始终运行Jenkins的备份。
...
如果您选择仅使用 Tomcat Server 运行 Jenkins，您可能会在`webapps`目录下找到`ROOT.war`而不是`jenkins.war`。请参阅第 2 章、*安装Jenkins中的*在 Apache Tomcat 服务器上单独安装Jenkins*一节。*
...
10.  登录到您的Jenkins实例。要确认Jenkins版本，请查看您的Jenkins仪表板的右下角，在那里您会发现一个新的Jenkins版本号。

# 升级运行在 Windows 上的独立Jenkins
...
2.  在我们升级Jenkins之前，备份我们的`jenkins_home`目录是很重要的。参考*Jenkins备份和恢复*部分下的*创建Jenkins备份*部分。

在升级Jenkins之前，请始终运行Jenkins的备份。

在Jenkins独立实例(运行在 Windows 机器上)上，`jenkins.war`文件存在于`jenkins_home`目录中。因此，备份`jenkins_home`目录就足够了。

3.  接下来，停止Jenkins服务。为此，从窗口运行中执行`services.msc`。这将打开窗口服务页面。
4.  搜索Jenkins服务(通常名为Jenkins)。停止 Jenkins 服务，如下图所示:
...
9.  登录到您的Jenkins实例。要确认Jenkins版本，请查看Jenkins仪表板的右下角，在那里您应该会看到一个新的Jenkins版本号。
...
要下载Jenkins(LTS)的特定版本，请访问以下链接:[http://mirrors.jenkins.io/war-stable/](http://mirrors.jenkins.io/war-stable/)并选择所需版本的Jenkins(例如，[http://mirrors.jenkins.io/war-stable/2.73.1/jenkins.war](http://mirrors.jenkins.io/war-stable/2.73.1/jenkins.war))。

要下载Jenkins的特定版本(每周)，请访问以下链接:[http://mirrors.jenkins.io/war/](http://mirrors.jenkins.io/war/)并选择所需版本的Jenkins(例如，[http://mirrors.jenkins.io/war/2.78/jenkins.war](http://mirrors.jenkins.io/war/2.78/jenkins.war))。

3.  在我们升级Jenkins之前，备份我们的`jenkins_home`目录是很重要的。参考*Jenkins备份和恢复*部分下的*创建Jenkins备份*部分。
...
以下部分仅适用于使用`jenkins_home`目录的数据卷运行 Jenkins 实例的情况。参见[第 2 章](02.html)、*安装Jenkins*中的*在 Docker 上运行Jenkins、使用数据卷运行Jenkins容器*部分。
...
3.  您应该会得到类似于前面代码片段的输出。注意Jenkins容器的名字，在我的例子中是`jenkins_prod`。
...
5.  使用以下命令列出 Docker 主机上可用的 Docker 映像。你可以看到我们有一个Jenkins·多克的图像:`jenkins/jenkins:lts`。然而，这已经不是最新的了:
...
上述命令可能需要一段时间来下载Jenkins Docker 映像。

在撰写本章时，2.73.1 是最新的 Jenkins 版本(LTS)。通过修改命令选择所需的Jenkins版本。

7.  下载完成后，再次执行`sudo docker images`命令，如下图所示。请注意新的Jenkins·多克图像。在我的例子中，是`jenkins/jenkins:2.73.1`:
...
10.  登录到您的Jenkins实例。你应该看到你所有的工作/设置完好无损。要确认Jenkins版本，请查看您的Jenkins仪表板的右下角，在那里您会发现一个新的Jenkins版本号。

# 用户管理员

让我们看看Jenkins在用户管理领域能提供什么。在Jenkins仪表板中，单击管理Jenkins|配置全局安全性以访问配置全局安全性页面。
...
在“配置全局安全性”页面上，您将看到“启用安全性”选项已经启用。启用安全选项应始终打开；禁用它将使任何拥有Jenkins网址的人都可以访问Jenkins，没有任何限制。
...
默认情况下，Jenkins自己的用户数据库选项处于启用状态。我们在 Jenkins 设置向导中创建的初始用户都存储在 Jenkins 自己的用户数据库中。没有任何类型的实际数据库，所有用户信息都保存为 XML 文件。让我们快速了解一下每种身份验证方法。

# 委托给 servlet 容器

仅当您从 servlet 容器(如 Apache Tomcat 等)运行 Jenkins 服务器时，才能使用此选项。启用此选项将允许 Jenkins 使用 servlet 容器的领域对用户进行身份验证。

例如，在第 2 章、*安装Jenkins*、的*在 servlet 容器*内运行Jenkins子部分*中，我们修改了`tomcat-user.xml`文件来创建用户和访问。这就是`UserDatabaseRealm`的一个例子。*
...
还有一个选项允许用户在登录页面注册。要启用它，勾选Jenkins自己的用户数据库下的**允许用户注册**选项。
...
作为新用户，当你点击**创建账户**链接时，系统会要求你填写一些关于自己的基本信息，比如用户名、密码、邮箱、全名等等。一旦你填写完必要的信息，你将被允许进入Jenkins。
...
1.  从Jenkins仪表盘，点击**管理Jenkins|管理用户**。
...
*   **我的视图**页面将带您进入当前用户可访问的视图。如果没有为当前用户配置视图，那么**我的视图**页面将显示默认的全部视图(Jenkins仪表盘)。
...
要访问Jenkins授权设置，从Jenkins仪表板导航至**管理Jenkins|配置全球安全|访问控制**。
...
这是Jenkins在安装和设置新的Jenkins服务器时附带的默认身份验证设置。名称不言自明，即登录用户默认为管理员。同样，不建议使用此选项。

在“登录用户可以做任何事情”字段下，有一个名为“允许匿名读取访问”的选项(默认情况下禁用)。勾选(启用)此选项后，任何有权访问Jenkins网址的人都将立即被带到Jenkins仪表板，对所有Jenkins职务具有只读访问权限。但是，您需要登录才能编辑Jenkins作业或查看Jenkins的配置。

# 矩阵安全

这是Jenkins最广泛使用的授权方法之一。让我们通过执行以下步骤来详细探索它:
...
1.  要访问Jenkins授权设置，从Jenkins仪表板导航至**管理Jenkins|配置全球安全|访问控制**。
...
6.  接下来，在Jenkins仪表板上右键单击任何Jenkins作业，并选择**配置**。
...
在本章中，我们看到了如何在 Jenkins 中配置一些基本但重要的元素，所有这些都借助于一些实际的例子。Jenkins升级、Jenkins备份和Jenkins用户管理是我们在本章中学到的一些重要内容。

下一章是关于Jenkins主从架构和Jenkins*分布式构建系统*。*