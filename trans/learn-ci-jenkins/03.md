...
2.  一直向下滚动到 Maven 部分，然后单击添加 Maven 按钮。您将看到一个选项列表，如下图所示。按如下方式填写信息:
    ...
    2.  默认情况下，将显示“从 Apache 安装”选项。这将使Jenkins从 Apache 下载 Maven 应用:
...
Jenkins蓝海是一种全新的与Jenkins互动的方式。它更像是Jenkins主应用的用户界面助手。以下是Jenkins蓝色海洋的一些特点:
...
您使用经典的Jenkins界面创建的管道可以在新的Jenkins蓝色海洋中可视化，反之亦然。正如我之前所说的，Jenkins蓝海是Jenkins主应用的用户界面助手。
