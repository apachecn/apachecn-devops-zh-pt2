...
本章将详细介绍如何使用现有的 Jenkins 实例设置 Jenkins Blue Ocean，以及如何使用 Docker 设置它。我们将详细查看蓝色海洋**用户界面** ( **用户界面**)并讨论詹金斯经典视图和蓝色海洋视图之间的差异。我们还将研究管道语法，并简要讨论其用途，解释两种不同类型的管道语法。
...
管道视图的功能也不同于它们在詹金斯 1.0 中的功能。管道阶段视图还帮助我们可视化管道中的各个阶段。

# 为什么要迁移到 Jenkins 2.0？

那么，首先，为什么完全转向 Jenkins 2.0，而不继续使用 Jenkins 1.0 呢？詹金斯经典视图被认为是杂乱的，没有考虑到易用性。Jenkins 2.0 以更直观的方式大力推动了 Docker Images 的使用。此外，新的用户界面包括一个詹金斯管道编辑器，并通过引入管道视图来改变您找到构建的方式。新用户界面的目标是减少混乱，提高使用詹金斯的团队每个成员的清晰度。新的用户界面还具有 GitHub 和 Bitbucket 集成以及 Git 集成。詹金斯 2.0 用户界面本质上是你安装的插件的集合，叫做蓝海。
...
2.  从詹金斯主页或詹金斯经典版的仪表板中，单击仪表板左侧的**管理詹金斯**
3.  接下来点击管理詹金斯页面中心的**管理插件**
4.  单击“可用”选项卡，并在“过滤器”文本框中键入`Blue Ocean`，该文本框将插件列表过滤到名称和/或描述包含“蓝色”和“海洋”字样的插件

更多信息请阅读[第 7 章](07.html)、*开发插件*，具体为*安装詹金斯插件*章节。

# 通过詹金斯 Docker 映像安装蓝海插件

您需要确保安装了 Docker，以便获得詹金斯 CI Docker 映像。
...
使用 Windows 命令提示符或 OS X/Linux 终端应用程序检查 Docker 版本，确保安装了 Docker。在命令行中运行以下命令:

![](assets/71166b35-4c61-4c35-8da1-d93477645440.png)
...
![](assets/75f86bc1-49d1-46fe-b27e-6cf0c368a2ed.png)
...
![](assets/1095a144-6ed5-4b66-8f19-dcd414e930c8.png)

您可以通过简单地创建一个 shell 脚本或创建一个别名来简化这一过程。

这是我在文本编辑器中创建的一个 shell 脚本:

![](assets/ae753d6a-1d5e-4f9c-a5ee-e10d275e3071.png)
...
一旦 Jenkins 正在运行，您可以转到这里:`http://localhost:8080` ，您将需要通过提供为管理员生成的默认密码来解锁 Jenkins。在[第 5 章](05.html)、*詹金斯*的安装和基础知识中，我们跳过了安装建议插件的入门步骤，但这次我建议您在入门屏幕上安装建议插件:

![](assets/1317bc5f-f095-4bfa-af7a-1ccae703e3c4.png)

通过点击安装建议插件按钮，您将获得所有的建议插件和相关插件，这将有助于您在新的詹金斯 2.0 流程中使用管道等。

# 访问蓝海詹金斯视图

您需要确保单击“打开蓝色海洋”按钮，该按钮看起来类似于:

![](assets/badaaee4-6cfc-4e8b-b903-2084093bdf24.png)

一旦你点击打开蓝色海洋按钮，你将被重定向到这个网址:`http://localhost:8080/blue/organizations/jenkins/pipelines`。詹金斯用户界面看起来会非常不同，行为也会不同。

这是您将看到的初始屏幕，因为我们尚未创建任何管道:

![](assets/cfc6c32e-e7bd-417b-a6e9-af003b4b6d79.png)

We will explore the Pipeline Syntax and how to navigate the Jenkins 2.0 UI in the following sections.

# 詹金斯管道

我们将使用詹金斯 2.0 用户界面创建我们的第一个管道，并将使用内置于新的詹金斯 2.0 用户界面中的管道编辑器创建一个詹金斯文件。

# 创建詹金斯管道

我们要做的第一步是单击“创建新管道”按钮。您将被重定向到以下屏幕:

![](assets/af4a51b2-db74-49c2-a980-cd22ecf1210e.png)
...
![](assets/e8dd5456-9d1a-4bc4-8b5c-1b67f9370adf.png)

单击“在此创建访问密钥”链接后，您将被重定向到以下 GitHub 页面:

![](assets/ec54ab3d-fec2-4178-ad76-4be2a213ebf9.png)

您可以选中默认选项，然后单击标题为“生成令牌”的绿色按钮。请确保将此个人访问令牌保存在安全的地方，因为它只会显示一次；根据我们的需要复制它。您需要将访问令牌粘贴到“连接到 Github”输入框中，然后单击蓝色的“连接”按钮:

![](assets/186e26ea-6f1d-4a97-b138-a35240b48b9c.png)

# 选择您的 GitHub 组织

您需要选择您所属的 GitHub 组织。在下面的截图中，我选择了`jbelmont`的 GitHub 用户名组织:

![](assets/64d91c75-d3ce-4f41-8a33-536cfb691e72.png)

# 挑选 GitHub 存储库

您需要做的最后一步是实际选择想要在其中创建 Jenkins 管道的 GitHub 存储库。在这里的截图中，我输入了`cucumber-examples`并选择了下拉列表。然后蓝色的“创建管道”按钮被启用:

![](assets/78944deb-ff37-401d-87e4-9e90f1642745.png)

# 使用管道编辑器创建管道

在我们选择的 GitHub repo 中，没有现有的 Jenkinsfile，因此我们被重定向到管道编辑器屏幕，在那里我们可以创建第一个 Jenkinsfile:

![](assets/856f8223-249a-4543-bf92-f846bd823ff5.png)

我们需要为 Node.js 和代理添加一个 Docker 映像，如下所示:

![](assets/ffc8dbc0-9985-4bdf-9a2b-12f4f9b5bf0e.png)

请注意，我们使用 Docker 的`-v`选项给出了装载数据卷的图像和参数。

接下来，我们单击灰色加号按钮，我们将看到以下变化:

![](assets/ff130c0e-3369-43b9-ae93-69768a0a9b47.png)

接下来，在给舞台命名后，我们单击蓝色的“添加步骤”按钮。在本演示中，我们将选择构建:

![](assets/ae789def-1b34-4b57-b7a3-0f78388c684f.png)

接下来，我们需要为该步骤选择一个选项。我们将选择标题为 Shell 脚本的选项，这将安装我们所有的 Node.js 依赖项:

![](assets/5e9ff84e-31b0-409b-9a97-959ccdd4fcd0.png)

接下来，我们在 Shell 脚本中输入一些要运行的命令:

![](assets/f1710434-bea4-44f4-8da5-6a476bea92ea.png)

接下来，我们将再次单击灰色加号按钮，向管道中再添加一个阶段，现在看起来类似于这样:

![](assets/cff137da-a10d-4d95-a16f-e60953410f47.png)

接下来我们将为这个阶段输入一个名称，为了本章的目的，我们将选择`Cucumber Tests`:

![](assets/5a67c3d1-7812-4b5f-a45d-bf59d8c959d0.png)

接下来，我们为这个阶段添加一个步骤，我们将再次选择 Shell 脚本作为选项:

![](assets/0c2ff88c-ccd5-4f3b-bdad-0641f36dbdba.png)

最后，我们将单击保存按钮并提供一条提交消息，以便将此更改推送到我们的 GitHub 存储库:

![](assets/ad40105c-ecd9-4bbc-935b-eeae45804f7e.png)

一旦您单击蓝色的保存并运行按钮，詹金斯文件将被合并到主分支，管道将运行。

# 在詹金斯蓝色海洋中航行

詹金斯经典视图中您习惯使用的一些视图在詹金斯蓝色海洋中不可用。Jenkins Blue Ocean 背后的主要概念是使 Jenkins 内的导航更容易访问，并在更好的图标和页面导航方面改进 Jenkins UI。新詹金斯 UI 的很多灵感来源于《蓝海战略》一书，该书强调世界已经从功能性开发工具转向开发人员体验，新 UI 渴望改善詹金斯的开发人员体验。

# 管道视图

下面的截图描述了詹金斯蓝色海洋的管道视图。请注意，对于两个不同的 GitHub 存储库，我们有两个不同的管道。第二个管道是通过单击“新建管道”按钮并添加个人 base64([https://github.com/jbelmont/decode-jwt](https://github.com/jbelmont/decode-jwt))Golang 库来创建的，该库通过命令行工具解码 JSON 网络令牌:

![](assets/de1715a6-7613-40ab-80fb-645ce0617b5b.png)
...
![](assets/454eab55-7377-4988-bacd-029e3f56dd21.png)
...
![](assets/eb5b0717-6ce9-4442-9ead-dcccc2cce6e7.png)

詹金斯蓝色海洋视图仍在开发中，因此任何管理任务，如添加插件和添加安全信息，仍在詹金斯经典视图中完成。
...
在*黄瓜示例*存储库中，我们使用管道编辑器创建了一个詹金斯文件。您实际上可以不使用管道编辑器来编写詹金斯文件，尽管我建议使用它来调试管道脚本，因为编辑器有一些不错的特性。
...
所有有效的声明性管道都必须包含在一个管道块中，正如您在前面的詹金斯文件中看到的那样。
...
本章讨论了如何在现有的 Jenkins 实例中使用 Jenkins 蓝色海洋视图进行设置，以及如何使用 Docker 设置蓝色海洋视图。我们观察了许多不同的詹金斯蓝色海洋视图，并讨论了它们与詹金斯经典视图之间的一些差异。我们还讨论了管道语法和詹金斯文件。下一章将介绍 Travis CI 的安装和基本用法。

# 问题

1.  如果通过 Docker 安装 Jenkins，可以使用蓝海视图吗？
2.  为什么在蓝色海洋视图中使用管道编辑器很有用？
3.  詹金斯经典视图和蓝色海洋视图有什么不同？
4.  你能详细看看管道的每个阶段吗？
5.  蓝海视图可以处理詹金斯的管理任务吗？
6.  stages 的语法是什么？
7.  声明性管道语法需要包装在管道块中吗？
...
请查看*帕克特出版*的《扩展詹金斯([https://www.amazon.com/dp/B015CYBP2A](https://www.amazon.com/dp/B015CYBP2A))一书，了解更多关于詹金斯插件的信息。