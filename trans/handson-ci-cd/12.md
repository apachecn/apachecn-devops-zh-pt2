...
![](assets/c9637bd7-2562-420a-867a-34f214f89f8b.png)

单击绿色的“继续”按钮后，您将被重定向到如下页面:

![](assets/31f8559e-135d-4670-8f40-f53b3f7caac7.png)

您需要输入您的全名和密码，您在上一页中提供的电子邮件地址已经为您设置好了。单击绿色的“继续”按钮后，您将收到一封新 Bitbucket 帐户的验证电子邮件，如下所示:

![](assets/b7616f17-d59a-4178-9455-37f516fee550.png)

单击“验证我的电子邮件地址”按钮后，您将被重定向到以下页面:

![](assets/9d160f00-7ee5-4879-94fd-fad7888dcf0e.png)

您必须为新的 Bitball 帐户提供唯一的用户名，因为您不能使用任何现有的用户名。单击继续按钮后，您将进入以下页面:

![](assets/6dd4fcde-f653-4f92-bf8d-a82f2d446fa0.png)

您可以通过单击“跳过”按钮跳过此部分，也可以输入您的信息，然后单击“提交”按钮，您将进入以下页面:

![](assets/962a0a33-991a-44cd-a1ef-08b15b487288.png)

# 创建 CircleCI 帐户

为了开始使用 CircleCI，您需要创建一个 CircleCI 帐户，并且您可以使用您的 GitHub 登录凭据或您的 Bitbucket 登录凭据:

![](assets/39ad9180-338f-4ac7-befd-d36e61976b4b.png)

您需要单击“注册”按钮来创建一个新的 CircleCI 帐户，您将被重定向到以下页面:

![](assets/b4a94106-f9c2-4258-bd5e-eaa5fd407be3.png)

你可以用任何一种注册，但是我们会选择*用比特桶**注册。o**T3 一旦点击按钮，您将被重定向到以下页面:*

![](assets/078426cc-a695-40a3-be78-6d6d0be2fd85.png)

我们将单击“授予访问权限”按钮，然后我们将转到以下页面:

![](assets/1db8446d-a1c1-46a2-a864-616658e4c3cf.png)
...
![](assets/717943b4-8242-46fc-bdb4-0e0b400f053a.png)

单击“添加项目”按钮后，您将进入如下页面:

![](assets/dfe9f6fd-c8b0-4d42-b6aa-e8249a6f3489.png)

我们将单击`functional-summer` GitHub 存储库的“设置项目”按钮，并将被路由到如下页面:

![](assets/9b6a8138-7d43-47a7-a211-62aa256ae9b0.png)
...
![](assets/77b296fe-5b73-4e36-a9be-25072538959c.png)
...
![](assets/1c32c15c-a00b-4251-ab9c-17b5ab4c8ee2.png)

单击此按钮后，我们将被重定向到 GitHub 用户界面中的类似页面:

![](assets/62fe413c-6c1e-4b08-b1ec-13de1a2c5730.png)

将我们的文件夹名称输入为`.circleci`，然后输入`/`字符，然后将我们的文件命名为`config.yml`。完成后，它将如下所示:

![](assets/a76294cb-f5b4-4ebe-ba55-78230d524731.png)
...
![](assets/efbe44f4-10b2-4436-aa08-17ce1def5339.png)

我们需要做的最后一步是返回到 CircleCI 中的“添加项目”页面，并单击“开始构建”按钮来启动我们在 CircleCI 中新配置的项目:

![](assets/70141810-499d-45d8-9dac-a518c209c6ba.png)
...
![](assets/bb38ec4f-204b-4c1e-af9e-7409e2f3d227.png)

如果我们进一步向下滚动，我们将在 CircleCI 应用程序中看到构建的每个步骤:

![](assets/7eec3ceb-c381-4f02-97b1-e228ee94aadf.png)

我们将在后面的章节中更详细地解释这一点，但是每个步骤都可以扩展以显示该步骤的细节。例如，如果我们单击纱线测试步骤，我们将看到以下详细信息:

![](assets/e490c4ff-e4fb-4918-82a6-fc75c8c80c28.png)
...
![](assets/0cf57ed2-793b-4608-bf7b-6c949403d30b.png)

我们需要点击添加键按钮。这将打开一个模式，我们在其中输入标签和公钥的内容，如下所示:

![](assets/d43516f1-3396-4b0f-b269-50f13201a48b.png)
...
![](assets/8d4cfacc-ca43-4373-a253-5ccc64c523ac.png)

接下来，我们将单击存储库按钮，如下所示:

![](assets/0067be31-254a-45d4-b5c5-90b7ceda9c12.png)

接下来，我们将通过提供存储库名称来创建一个新的存储库，将我们的版本控制系统设置为 Git，然后通过单击创建存储库按钮来完成:

![](assets/e09cf57c-8f00-4248-beac-10b27055218d.png)

请注意，我们单击了可选的高级设置下拉菜单，并将我们的语言设置为 Java 编程语言。单击“创建存储库”按钮后，我们将被重定向到如下页面:

![](assets/b14b4643-df50-4252-a4c3-7e5828c9a0f6.png)
...
![](assets/86f5041d-52f3-4e25-a3d5-cdabdb6e5781.png)

这将触发我们对`java-summer`项目的第一次构建，并让 webhook 为存储库工作。一旦我们点击开始构建按钮，我们需要点击作业按钮，以便查看我们被触发的新构建:

![](assets/83ac58b8-7a05-4e9c-a227-685f38cae333.png)
...
![](assets/73a4fd33-6554-4c2a-997a-542296cebbb6.png)
...
![](assets/e082879b-17e6-4fbb-a726-131992ca9edc.png)

以下是构建作业的步骤:

![](assets/60506a6b-8f14-41a5-93ba-477a53144542.png)
