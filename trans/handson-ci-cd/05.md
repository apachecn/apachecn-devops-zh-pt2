...
本章将帮助您在 Windows、Linux 和 macOS 中安装 Jenkins。我们还将了解Jenkins用户界面的基础知识。
...
本章是关于将Jenkins用于竞争情报/竞争情报流程。在本章中，我们将不讨论配置项/光盘概念，因为我们正在设置环境以使用Jenkins。
...
在 Windows 操作系统中安装 Jenkins 相对容易；只需转到Jenkins下载页面([https://jenkins.io/download/](https://jenkins.io/download/)):
...
一旦安装了巧克力，你可以简单的运行`choco install jenkins`通过巧克力安装Jenkins。

# 在命令提示符窗口中启动和停止Jenkins
...
接下来，我们将Jenkins安装到 Ubuntu 上:

```
sudo apt-get install jenkins
```

# 在 Ubuntu 中启动Jenkins服务
...
1.  前往Jenkins下载网址([https://jenkins.io/download/](https://jenkins.io/download/))。
2.  滚动到页面底部，您应该会看到可以安装 Jenkins 的操作系统列表。
3.  点按“Mac OS X”按钮链接，您应该会进入如下页面:

![](img/6ecaa6d2-72ff-431e-9b47-341bc9f81ee3.png)

4.  单击浏览器窗口底部的`.pkg`文件，或者双击下载文件中的Jenkins`.pkg`文件:
...
运行此命令后，您只需访问`localhost:8080`，然后就可以按照我们在*解锁Jenkins首次登录*部分运行的相同步骤进行操作。

# 在本地运行Jenkins

以下是Jenkins主仪表板页面的截图。我们将详细检查每个项目:
...
5.  让我们为Jenkins创建一个简单的构建，打印出`Hello World`:
...
让我们回到Jenkins仪表板，单击管理Jenkins，然后单击管理插件。
...
现在如果你点击重启Jenkins**标志**，Jenkins将重启，然后系统会提示你登录。
...
现在我们将看看如何在 Jenkins 仪表板中添加环境变量。确保点击管理Jenkins，然后点击配置系统 **:**
...
现在我们将创建另一个名为**戈朗项目**的Jenkins构建作业:
...
本章讨论了Jenkins的安装和导航Jenkins用户界面的基础知识。下一章将详细介绍Jenkins仪表盘和用户界面。
...
请查看帕克特出版的*与Jenkins学习持续整合-第二版*([https://www.amazon.com/dp/1788479351](https://www.amazon.com/dp/1788479351))。