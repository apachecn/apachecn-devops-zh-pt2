...
在谈论安全最佳实践时，我们的最终目标应该是确保没有未经授权的第三方能够访问我们不希望他们访问的应用程序或基础架构的任何部分。

例如，我希望最终用户能够运行一个脚本，通过网页或移动应用程序直接发出的 HTTP 请求调用我的一个无服务器函数。然而，例如，我不希望同一个用户能够访问我的 Kubernetes 仪表板。
...
可以看到`certificate-authority-data`区有一个证书库。此证书用于验证您的群集，这意味着每当您运行如下命令时，它都会按预期返回节点列表:
...
因此，除非您有正确证书的副本，否则您无法连接到群集。别担心，您仍然可以通过运行以下命令来访问您的证书:
...
千万不要在任何地方发布(也就是比如签入 GitHub，或者和同事分享)。如果它落入坏人之手，那么不仅有人拥有您的证书副本，他们还拥有您的其他群集信息，这意味着他们所要做的就是将它放在本地计算机上，然后他们就可以自由地启动应用程序。此外，由于大多数基于云的 Kubernetes 安装都可以访问您的云提供商来启动支持服务，如负载平衡器、存储和潜在的额外节点，因此您可能会发现自己的账单相当大，而且集群也受到影响。
...
这两个工具都可以被配置为坐在 OpenFaaS 安装的前面并拦截请求，并且当被配置时，向最终用户呈现登录提示。保护 OpenFaaS 安装的另一种方法是使用公共云服务中的网络工具将其锁定到您的 IP 地址。这样做的缺点是，根据应用程序调用函数的方式，您可能无法完全锁定它。
...
正如你所看到的，我们得到了一个识别裂变应用编程接口和版本号的响应。从`~/.kube/config`文件中删除证书，并运行以下命令:

```
$ fission function list
```

我们仍然可以与我们的裂变装置互动；这意味着默认情况下，裂变没有身份验证，当我们使用推荐的安装过程时，默认情况下，API 会暴露给互联网:

![](images/983249ac-e7b1-4e91-bd31-1afc0ba9d089.png)

工作正在进行，以更安全的默认方式运送裂变；你可以在下面的 GitHub 期关注它的进展:[https://github.com/fission/fission/issues/22/](https://github.com/fission/fission/issues/22/)。
...
这将意味着您的`FISSION_URL`将类似于`http://localhost:1234`，而不是没有身份验证的外部可访问的网址。裂变开发人员正在将这个解决方案构建到裂变中，它应该成为 2018 年初版本中的默认配置。
...
传统上，监控服务器意味着密切关注运行在固定服务器上的应用程序的可用性。为此，我们的监控工具将整理关于 CPU、RAM 和磁盘利用率的信息，以及哪些服务正在运行、进程数量以及服务和服务器本身的可用性。

我们将在特定的阈值设置触发器，这样，例如，如果 CPU 负载增加，我们可以登录到服务器，并在所述 CPU 负载开始影响我们的应用程序性能之前进行一些调查。

如您所见，监视 Kubernetes 集群与此大不相同。根据设计，与集群一起运行的应用程序应该是容错的，并且也是高度可用的——事实上，我们在前面几章中运行的功能有时只有执行该功能所需的时间。
...
如您所见，我们有运行在我的集群的两个名称空间中的信息荚，然后我们有集群内的节点。接下来，我们有所有已经下载的图像，以及所有正在运行的容器的详细信息。
