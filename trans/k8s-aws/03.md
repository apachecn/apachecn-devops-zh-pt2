...
我们需要为`kubeadm`提供一个配置文件，以便在它将要启动的每个组件上启用 AWS 云提供商。在这里，我们还将`tokenTTL`设置为`0`，如下命令所示；这意味着为允许工作节点加入集群而颁发的令牌不会过期。这一点很重要，因为我们计划使用自动伸缩组来管理我们的员工，并且新节点可能会在一段时间后加入该组:
