...
对于希望在集群上运行其应用程序的用户来说，在修改清单以引用硬编码的标识之前，首先考虑配置应用程序所需的 EBS 卷是很难的，也很耗时。这意味着 pod 清单需要包含一个特定于在 AWS 上运行所讨论的应用程序的配置。理想情况下，我们希望尽可能多的配置可以在我们可能部署它的不同环境之间重用，以避免因不得不修改配置而引入错误的风险。
