...
有些任务非常简单，写剧本来完成它们是对时间的巨大浪费。此外，任何Ansible的临时命令都可以制作成剧本——在将命令添加到剧本管道或工作流之前，Ansible的用户总是可以尝试一些命令并验证它们的参数。这是快速排除故障和应用快速更新或修复的好方法。下面的例子展示了如何通过将分叉数量设置为 1(使用`-f`选项)一次重启一个 web 服务器副本。这将按主机应用重启命令:
...
*   `ansible_connection`:这个在`ssh`之间变化，默认连接；`local`，与本地机器通信；和`docker`，用于直接在依赖本地机器的 Docker 客户端的 Docker 容器中运行命令。我们将在第 8 章、*Ansible高级功能*中更详细地介绍Ansible Docker 的用法。
...
现在事情开始变得有趣了。使用 Ansible 行动手册，我们将能够实现配置管理、流程编排、资源调配和部署。Playbook scripting 以更有条理的方式使用 Ansible 即席命令，类似于 shell 脚本安排 shell 命令在系统上执行任务的方式，但比这更高级。Ansible的行动手册可以在裸机、虚拟环境或云中设置和配置复杂的环境。它可以对多层机器展开进行排序；应用系统、设备和应用程序补丁和修复程序；从主机或监控服务收集数据；并相应地向服务器、网络设备和负载平衡器发送即时动作。所有这些任务都可以委托给其他服务器。
...
在本章中，我们看到了 Ansible 如何用于运行快速简单的命令。然后，我们看了 Ansible 如何管理其主机库存，这有助于我们理解其剧本脚本。我们发现了行动手册是如何构建的，以及它们是如何用来编排任务的。在下一章中，我们将发现Ansible模块，并了解它们在行动手册中执行的所有任务中有多重要。我们将同时关注 Linux 和 Windows 系统模块、一些网络设备以及各种可视化和云管理器。
