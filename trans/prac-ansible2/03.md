...
然而，库存不仅仅是一系列自动化目标。可转换的清单可以几种格式提供；它们可以是静态的，也可以是动态的，它们可以包含定义 Ansible 如何与每个主机(或主机组)交互的重要变量。因此，他们应该有自己的一章，在这一章中，我们将对库存以及如何在您使用 Ansible 自动化基础架构时最大限度地利用它们进行实际探索。
