...
到目前为止，在本书中，为了简单起见，我们主要使用了特定的 Ansible 命令来帮助您理解基础知识。然而，Ansible 的命脉无疑是剧本，这是一种在创建有用结果的结构中的任务的逻辑组织(想想临时命令)。这可能是为了在新构建的虚拟机上部署 web 服务器，也可能是为了应用安全策略。它甚至可以处理虚拟机的整个构建过程！可能性是无穷的。正如我们已经介绍过的，Ansible行动手册旨在编写简单且易于阅读，它们旨在自我记录，因此将成为您的 IT 流程中有价值的一部分。
