...
我们要覆盖的最后一个平台是 Windows 10 Professional 算是吧。没有支持的方式在 Windows 机器上本地运行 Ansible 控制器。正因为如此，我们将把视窗子系统用于 Linux。

这是一项功能，在撰写本文时，该功能处于测试阶段，仅适用于 Windows 10 专业版用户。要启用它，首先需要启用开发人员模式。为此，请打开 Windows 10 设置应用程序，然后切换开发人员模式，这可以在更新和安全下找到，然后为开发人员。
...
该文件的大部分是标准的 NTP 配置文件，并添加了一些可翻译的部分。第一个补充是第一行:
