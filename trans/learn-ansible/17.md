...
使用网络客户端或桌面应用程序登录工作区后，从“管理”菜单中选择“管理应用程序”选项，如下图所示:

![](../images/00159.jpeg)

这将打开您的浏览器，并将您带到工作区的应用程序目录；从这里搜索`Incoming WebHooks`，然后点击添加配置。
...
*   `username`:我们可以覆盖发布更新的用户名，因为我只是把它保持为可解释的
...
第一个问题是我们没有传递主机清单文件，所以会有警告说只有 localhost 可用；这很好，因为我们无论如何都只想在我们的可移植控制器上运行`say`模块:
...
这完美地总结了我和你的 Ansible 的经历。一旦您了解了基础知识，就很容易快速前进，开始构建越来越复杂的行动手册，这不仅有助于部署基本代码和应用程序，还可以部署复杂的云甚至物理架构。
