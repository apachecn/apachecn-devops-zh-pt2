...
vCloud Air 是 VMware 的**基础架构即服务** ( **IaaS** )产品，我说*是*，因为 vCloud Air 业务部门和负责该服务的团队在 2017 年年中被法国托管和云公司 OVH 从 VMware 手中收购。有三个可移植模块提供对 vCloud Air 的直接支持，以及 **VMware vCloud 混合服务** ( **vCHS** )和 **VMware vCloud 控制器** ( **vCD** )。
...
本模块用于创建和管理虚拟应用程序。vApp 是一个或多个虚拟机，它们组合起来提供一个应用程序:
...
VMware tools 是在虚拟机内部运行的应用程序。一旦启动，它允许 VMware 与虚拟机交互，从而允许`vmware_guest_file_operation`和`vmware_vm_shell`等模块运行。
...
当我运行行动手册时，运行时间大约为 30 分钟，但在运行结束时，我启动了七台虚拟机并可供使用，因此我能够运行引导环境就绪的行动手册集，这样我就可以将它们交给客户，让他们部署应用程序。
