...
*   **亚马逊简单队列服务(亚马逊 SQS)** :该服务主要为 web 应用程序提供消息队列
...
*   一个安全组，允许从应用程序负载平衡器访问端口`80` (HTTP)到 EC2 实例
*   一个安全组，允许从 EC2 实例访问亚马逊 RDS 实例上的端口`3306` (MySQL)
*   一个安全组允许从 EC2 实例访问亚马逊 EFS 卷上的端口`2049` (NGF)

这将为我们提供基本网络，允许对除应用程序负载平衡器之外的所有内容进行限制性访问，我们希望应用程序负载平衡器公开可用。在我们开始创建部署网络的 Ansible 行动手册之前，我们需要获得一个 AWS API 访问密钥和秘密。

# 创建访问密钥和秘密

为您自己的 AWS 用户创建一个访问密钥和秘密密钥是非常可能的，这将使 Ansible 能够完全访问您的 AWS 帐户。
...
如您所见，成功消息告诉您，这是您最后一次能够下载凭据，这意味着您将无法再次看到秘密访问密钥。单击“显示”按钮并记下密钥，或者单击“下载”。csv 按钮；您将没有机会恢复秘密访问密钥，只能让它过期并生成新密钥。

现在，我们有了一个用户的访问密钥 ID 和秘密访问密钥，该用户拥有使用 Ansible 启动我们的 VPC 所需的权限，我们可以开始编写剧本了。

# VPC 剧本

我们需要讨论的第一件事是如何以安全可靠的方式将我们的访问密钥 ID 以及秘密访问密钥传递给 Ansible。由于我将在 GitHub 上的公共存储库中共享最终剧本，我不想与世界共享我的 AWS 密钥，因为这可能会变得昂贵！通常，如果它是一个私有存储库，我会使用 Ansible Vault 来加密密钥，并将它们与其他潜在的敏感数据(如部署密钥等)一起包含在其中。
...
现在我们已经配置了基本的 VPC，我们可以开始在其中启动服务，从应用程序负载平衡器开始。

# ELB 的角色

我们将在本章中看到的最后一个角色是启动应用程序负载平衡器。它会创建一个目标组，然后将其连接到应用程序负载平衡器。我们将用这个角色创建的负载平衡器是基本的；我们将在后面的章节中详细讨论。
...
在这一章中，我们已经采取了下一步，使用 Ansible 在公共云中启动资源。我们创建了一个 VPC，设置了应用程序所需的子网，提供了一个互联网网关，并设置了实例通过它路由输出流量，从而为自动化相当复杂的环境奠定了基础。
