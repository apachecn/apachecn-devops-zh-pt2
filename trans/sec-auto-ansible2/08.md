...
下面的截图是克莱尔扫描仪在为请求的Docker图像行动。正如你所看到的致命错误，所以当它发现 docker 图像有任何问题时，它会返回错误，我们可以使用`ignore_errors`来处理它。
...
使用`docker_image`模块在本地拉动Docker容器:
...
我们也可以访问`vuls`服务器 IP 地址的 web UI 界面，以表格和Ansible的格式查看详细结果。这对于大规模管理大量服务器和补丁非常有用:
