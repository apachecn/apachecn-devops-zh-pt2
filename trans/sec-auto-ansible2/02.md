...
这基本上允许您使用易于使用的网络图形用户界面设计您的行动手册工作流等，并增加了从另一个配置项/光盘工具(如Jenkins)调用它的灵活性。顺便说一句，Jenkins是下一个需要安装和学习的软件。
...
要设置Jenkins，请运行以下命令。其中`192.168.1.7`为Jenkins将要安装的服务器 IP 地址:

```
ansible-playbook -i '192.168.1.7,' site.yml --ask-sudo-pass
```

现在，我们可以配置 Jenkins 安装插件，运行计划的作业，并做许多其他事情。首先，我们必须通过浏览到`http://192.168.1.7:8080`并提供自动生成的密码来导航到Jenkins仪表板。如果剧本运行没有任何错误，它将在剧本结束时显示密码:

![](img/b53f3348-ad91-47a6-8fed-99f1792e07ce.png)

通过填写详细信息并确认登录Jenkins控制台来创建新用户:
...
现在我们已经准备好为Jenkins使用 Ansible 插件了。在主仪表板中创建新项目，为其命名，然后选择自由式项目继续:
...
前面的截图是一个调用 Ansible 即席命令的构建示例。这可以修改为基于特定事件的Ansible剧本或任何其他脚本。
...
在Jenkins中，这是一个更通用的工具，我们可以通过使用插件来扩展它的功能。角色策略插件是一个社区插件，用于管理Jenkins的角色。使用它，我们可以为用户和组创建不同的访问级别控制:
...
默认情况下，Jenkins作为普通的旧 HTTP 运行。为了启用 HTTPS，我们可以在Jenkins前面使用一个反向代理，比如 Nginx，来充当 HTTPS。
...
我们正在使用Jenkins的默认凭据功能。这将在本地文件系统中存储密钥和密码。Jenkins也可以使用不同的插件来处理这个问题，比如[https://wiki.jenkins.io/display/JENKINS/Credentials+Plugin](https://wiki.jenkins.io/display/JENKINS/Credentials+Plugin)。
...
Rundeck 还提供了 RBAC 作为Ansible塔。与 Tower 不同，这里我们必须使用`/etc/rundeck/`中的 YAML 配置文件进行配置。
...
Jenkins 为管理报告提供了标准输出和 REST API。Jenkins有一个非常大的社区，有多个插件可用，如 HTML 发布者插件和黄瓜报告插件。
