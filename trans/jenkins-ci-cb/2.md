...
*   **A2-跨站点脚本编写(XSS):** 当应用向客户端浏览器返回非转义输入时，可能会发生 XSS 攻击。默认情况下，Jenkins管理员可以通过职务说明做到这一点。
...
渗透测试有可能损坏正在运行的应用。确保您有Jenkins工作区的备份副本；您可能需要重新安装。也请关闭Jenkins内任何已启用的安全性；这允许 w3af 在安全表面上自由漫游。
...
备份你的牺牲Jenkins服务器，并关闭其安全性。预计攻击结束时应用会不稳定。
...
模糊器擅长覆盖应用的很大一部分网址空间，从而触发错误，从时间上来说，搜索这些错误是很昂贵的。考虑通过Jenkins工作实现自动化，作为项目质量保证过程的一部分。
...
Groovy 脚本以用户身份运行 `jenkins`。这个食谱强调了Jenkins应用的力量和危险。
...
耶鲁 CAS([http://www.jasig.org/cas](http://www.jasig.org/cas))是一个单点登录服务器。它被设计为校园范围的解决方案，因此易于安装，配置也相对简单，可以满足您特定的基础设施需求。CAS 允许您登录一次，然后自动使用许多不同的应用，而无需再次登录。这是为了一个典型的Jenkins用户在一天中使用的一系列应用之间更愉快的用户交互。
...
祝贺您，您有了一个可以工作的单点登录，在这个单点登录中，Jenkins可以与大量其他应用和身份验证服务无缝地发挥作用。
