...
该方法运行在名为*的方法中开发的网络应用，基于 JSP 语法错误*使Jenkins作业失败，通过在测试运行前启动服务器，然后关闭服务器，将 Jetty 与集成测试捆绑在一起。该生成创建自签名证书。为 HTTP 和安全 TLS 流量定义了两个 Jetty 连接器。要创建远程登录端口，还需要定义 `shutdown`命令。
