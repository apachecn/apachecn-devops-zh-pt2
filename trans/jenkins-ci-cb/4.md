...
*   **跟踪你的受众:**如果你是公开交流，你应该跟踪使用模式，这样你才能改善服务。考虑将您的Jenkins页面连接到谷歌分析或开源分析应用 Piwik。
...
当您在应用中更新样式表时，在您清理浏览器缓存之前，您可能看不到浏览器中的更改。Jenkins以一种巧妙的方式避开了这个延迟问题。它使用一个带有唯一号码的网址，该号码随每个Jenkins版本而变化。例如，对于 `css`目录，您有两个 URL:
...
Jenkins 插件可以将构建历史推送到不同的知名社交媒体服务。两个主要服务是**谷歌日历**(用于议程)和**推特**。现代安卓或 IOS 移动设备已经为这两种服务预装了应用，降低了采用的障碍。在这个食谱中，我们将配置Jenkins使用谷歌日历。
...
开源代码是可延展的；您可以下载、修改、提交和审查代码，并对其质量形成自己的判断。推特频道对经理们来说是一个很好的机会，他们可以看到成功和失败的历史，因为他们试图形成一个关于产品路线图是否现实的观点。大多数现代社交设备，如安卓或 IOS 移动设备，都内置了推特应用。唯一需要的配置是您的用户帐户信息。这个食谱概述了如何让Jenkins发推。
...
有许多丰富的移动应用可以通知Jenkins的工作状态。这个食谱会把你指向他们的主页，这样你就可以选择你最喜欢的。
...
大多数应用使用Jenkins的 RSS 源获取信息，如`/rssLatest`和`/rssFailed`，然后通过移动网络浏览器加载链接的页面。除非Jenkins网址配置正确，否则链接将断开，浏览器将返回 404 页未找到错误。
