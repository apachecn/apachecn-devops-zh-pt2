# [*第九章*](#_idTextAnchor116) :用 Skaffold 创建生产就绪 CI/CD 管道
...
我们已经使用 GitHub Actions 成功地构建了一个 Spring Boot 应用。下一节将使用 Skaffold 和 GitHub Actions 从 GitHub 存储库中为 Spring Boot 应用创建 CI/CD 管道。
...
在本章中，您已经学习了如何使用 GitHub Actions 来自动化您的开发工作流。我们从解释 GitHub 动作及其组件开始这一章。我们用一个例子解释了 GitHub Actions 和相关概念。在这个例子中，我们解释了如何从 GitHub 存储库中构建、测试和部署您的 Java 应用。然后我们描述了如何使用 Skaffold 和 GitHub Actions 为 Kubernetes 应用创建 CI/CD 管道。
