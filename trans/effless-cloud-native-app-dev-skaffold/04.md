...
综上所述，在这里，我们指示Skaffold用一个不同的`Dockerfile`来代替用来构建第一个`docker.io/hiashish/skaffold-example`映像的`Dockerfile`，命名为`Dockerfile_dev`。
...
在前面的日志中，您可以看到，首先，Skaffold开始使用 Docker 构建我们的映像。此外，我们使用了多阶段构建，然后在步骤 1 到 6 中，我们进入构建的第一阶段，其中我们在容器内创建了应用的`jar`:
