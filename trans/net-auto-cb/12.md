...
AWX 为 RBAC 提供了 AWX 界面中的所有对象，如Ansible剧本、Ansible目录和机器凭证。这个 RBAC 提供了关于谁可以在 AWX 创建/编辑/删除不同组件的细粒度控制。这为将简单的自动化任务委托给操作团队提供了一个非常强大的框架，设计团队可以专注于开发行动手册和工作流。AWX 能够定义不同的用户，并根据他们的工作角色为他们分配权限。
