...
Ansible 存储它使用 Ansible 变量管理的节点的信息。可转换变量可以在多个位置声明。然而，在观察 Ansible 的最佳实践时，我们将概述 Ansible 为清单文件中声明的节点寻找变量的两个主要部分。
...
为了遵循此配方，必须存在一个 Ansible 库存文件，并按照前面的配方进行配置。此外，我们所有主机的可转换变量应该按照前面的方法进行定义。
