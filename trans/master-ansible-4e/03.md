...
有鉴于此，使用 Vault 在单个主密码下保护任何秘密似乎是不合理的。Ansible 2.4 引入了 Vault id 的概念作为解决方案，虽然目前旧的单一密码命令仍然全部有效，但建议在命令行上使用 Ansible 时使用 Vault id。每个保管库标识必须有一个与之关联的密码，但是多个机密可以共享同一个标识。
