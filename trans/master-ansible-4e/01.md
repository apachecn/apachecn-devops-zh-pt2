...
当处理这些主机或与这些主机运行的应用程序相关的特定于站点的数据时，这些类型的变量可用于定义Ansible行为。一个变量无论来自`host_vars`还是`group_vars`，都会分配给一个主机的`hostvars`，可以从剧本和模板文件中访问。访问一个主机自己的变量可以通过简单的引用名称来完成，比如`{{ foobar }}`，访问另一个主机的变量可以通过访问`hostvars`来完成；例如，要访问`examplehost`的`foobar`变量，可以使用`{{ hostvars['examplehost']['foobar'] }}`。这些变量具有全局范围。
