...
要找到Ansible Python 代码的路径，只需键入`python3 -c "import ansible; print(ansible)"`。请注意，和我一样，您可能同时安装了 Python 2 和 Python 3——如果您不确定 Python Ansible 在哪个版本下运行，您将需要执行版本 2 和版本 3 的二进制文件来发现您的模块位置。
...
图 9.9–在Ansible核心库存管理器代码中添加 pdb 断点
...
图 9.21–将远程 Python 调试器插入Ansible模块代码
