...
OpenShift Online 是来自红帽的公共 PaaS——应用开发和托管平台。它自动化了应用的调配和取消调配、管理和扩展过程。这支持命令行客户端工具和网络管理控制台，可以轻松启动和管理应用。Jenkins应用是由 OpenShift Online 提供的。OpenShift Online 有一个免费的计划。
