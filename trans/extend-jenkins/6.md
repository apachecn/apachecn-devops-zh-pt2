...
*   使用集成开发环境进行更改并运行Jenkins
*   在集成开发环境中运行和调试Jenkins和插件代码的基础知识

我们将从建立我们的发展环境开始；然后，按照传统，我们将创建一个非常简单的`Hello World` Jenkins 插件项目来说明机制并让我们开始。

本章的大部分内容基于Jenkins插件教程指南中的主题:
...
我们现在需要告诉 T2·马文关于Jenkins的事情；它在哪里，如何建造。我们通过用前面提到的Jenkins插件教程页面的**设置环境**部分中提供的 XML 更新 m2 主目录中的`settings.xml`文件来做到这一点:
...
我们还没有写一行代码，但我们刚刚生产了我们的第一个Jenkins插件！

现在让我们将其部署到一个标准的Jenkins实例中:

1.  打开Jenkins实例的主页。
2.  导航至Jenkins**主页** | **管理Jenkins**。
3.  选择**管理插件**，然后选择**高级**。
...
在Jenkins实例上安装并运行我们自己的插件看起来很酷，第一次这样做很有趣。然而，当你在开发一个插件的时候，每次你做一个小的改变都要经历这样的过程，这比你想要的要麻烦的多！
...
为了让生活变得更容易，我们可以建立一个 IDE 来帮助我们开发Jenkins插件。

### 注

官方的Jenkins插件教程页面在 https://wiki.jenkins-ci.org/display/JENKINS/Plugin+tutorial T2。本教程包含 NetBeans、IntelliJ IDEA 和 Eclipse 的步骤。前两个非常简单，所以我们将在这里更详细地介绍 Eclipse 设置。
...
在这里，我们可以看到通过Jenkins运行构建时断点被激活。此时，焦点将自动从浏览器中的 Jenkins 切换到 Eclipse IDE，在这里我们可以在运行时检查现有变量的当前值。然后，我们可以通过实时调试值并在每一步监控控制台输出，一步一步地浏览代码。

这是一个非常方便的开发特性，以这种方式设置您的 Jenkins 插件开发环境可以让事情变得更有效率，并且让您的生活更加轻松！

# 总结

在本章中，我们已经构建、打包并部署了我们自己的“基本”Jenkins插件。
