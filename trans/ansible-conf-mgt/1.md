...
可移植模块努力防止在不需要时进行更改。这被称为幂等性，可以使针对多台服务器运行命令的速度更快。不幸的是，Ansible 无法知道您的命令是否改变了什么，所以为了帮助它变得更幂等，您必须给它一些帮助。它可以通过`creates`或`removes`参数来实现。如果给出一个`creates`参数，如果文件名参数存在，命令将不会运行。`removes`论点则相反；如果文件名存在，命令将运行。
