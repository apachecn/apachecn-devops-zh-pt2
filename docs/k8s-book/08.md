...
现在假设您将要部署一个需要 10GB 存储的应用。太好了，你已经有三个 10GB 的 PVs 了。为了让应用使用其中一个，它需要一个聚氯乙烯。如前所述，聚氯乙烯就像一张票，让一个吊舱(应用)使用光伏。一旦应用有了聚氯乙烯，它就可以将相应的光伏组件作为一个卷安装到其吊舱中。如果需要可视化表示，请参考图 8.2。
...
通常情况下，您会通过更高级别的控制器来部署您的应用，例如*部署*和*状态集*，但是为了简单起见，您将部署单个 *Pod* 。像这样部署的吊舱通常被称为*“单件”*，不建议用于生产，因为它们不能提供高可用性，也不能自我修复。
...
存储类通过允许应用动态请求存储，将事情推向了一个新的高度。您可以从存储后端创建引用存储类或存储层的存储类对象。一旦创建，存储类将监视应用编程接口服务器，寻找引用该存储类的新持久卷声明。当匹配的聚氯乙烯到达时，供应链动态创建存储，并使其作为光伏可用，可作为卷装入吊舱(容器)。`````````````````````**