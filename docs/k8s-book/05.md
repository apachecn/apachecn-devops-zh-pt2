...
*声明模型*是一种告诉Kubernetes你的*期望状态*是什么的方式，而不需要进入*如何*实现它的细节。你把*怎么*留给Kubernetes。
