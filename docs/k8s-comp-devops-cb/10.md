...
本章中的食谱期望您按照[第 1 章](01.html)*中描述的推荐方法之一，构建生产就绪型Kubernetes集群*，部署一个功能性Kubernetes集群。

本章中的*詹金斯CI/CD环境的日志记录*方法期望您拥有一个功能性的詹金斯服务器，该服务器具有通过遵循[第 3 章](03.html)、*构建CI/CD流水线中描述的推荐方法之一创建的现有配置项管道。*
...
1.  获取在`kube-system`命名空间中运行的POD列表。在这个命名空间中运行的POD，尤其是`kube-apiserver`、`kube-controller-manager`、`kube-dns`和`kube-scheduler`，在Kubernetes控制平面中扮演着关键角色:
...
2.  从`kube-system`命名空间中的单个容器中查看日志。在这个例子中，这个POD是`kube-apiserver`。替换POD的名称，并根据需要对其他POD重复此操作:
...
3.  豆荚可以容纳多个容器。列出容器时，`Ready`栏下的数字显示容器内的容器数量。让我们从`kube-system`命名空间中有多个容器的 pod 中查看一个特定的容器日志。在这里，我们正在看的POD叫做`kube-dns`。替换容器的名称，并对具有多个容器的任何其他容器重复此操作:
...
3.  查看`postgres.yaml`文件，并将其应用于创建 PostgreSQL 状态集。我们可以用它来部署POD和自动创建光伏/聚氯乙烯:
...
让我们执行以下步骤来部署Kubernetes ( **ECK** )上的**弹性云:**
...
*   **Kubernetes上的弹性云**(**ECK**):[https://github.com/elastic/cloud-on-k8s](https://github.com/elastic/cloud-on-k8s)
...
确保您已经准备好了一个**谷歌Kubernetes引擎** ( **GKE** )集群，并配置了`kubectl`来管理集群资源。如果没有，可以按照*在谷歌云平台*上配置 Kubernetes 集群食谱中[第 1 章](01.html)、*构建生产就绪的 Kubernetes 集群*中的说明进行操作。
...
3.  在“集群配置”页面上，确保禁用了旧堆栈河日志记录和旧堆栈河监控，并且将“堆栈河Kubernetes引擎监控”选项设置为“已启用”:
