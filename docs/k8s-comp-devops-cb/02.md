...
在本章中，我们将讨论可用于在 Kubernetes 上部署云原生应用的资源调配工具。您将学习如何使用最流行的生命周期管理选项在 Kubernetes 上部署 DevOps 工具和 **CI** / **CD** (简称**持续集成** / **持续交付**或**持续部署**)基础架构。您将获得执行第一天和一些第二天操作的技能，例如安装、升级和版本控制部署、排除新应用以及在不再需要部署时删除部署。
...
在本节中，我们将创建在 Kubernetes 中部署应用所需的资源配置。您将学习如何使用**和另一种标记语言** ( **YAML** )文件创建 Kubernetes 清单、部署工作负载和推出新版本。
...
本食谱将指导您使用清单文件创建一个部署，以保持一组 POD 运行。部署用于声明一个 pod 应该运行多少副本。部署可以放大和缩小；稍后我们将在[第 7 章](07.html)、*扩展和升级* *应用*中看到更多关于该主题的内容。
...
运行上述命令后，将从容器注册表中提取 YAML 清单中提到的容器映像，并在部署清单中定义的 Kubernetes 集群中调度应用。现在，您应该能够按照下一个方法验证部署。
...
现在，您已经验证了新部署已成功部署并运行。在生产环境中，您还需要编辑、更新和扩展现有应用。在下一个方法中，您将学习如何在现有部署上执行这些修改操作。
...
3.  通过创建一个新的副本集并将旧的副本集从`2`缩小到`0`，确认您的部署加速了新的 POD:
...
6.  通过将更新后的 YAML 清单应用于您的更改来更新部署。这一步将应用用于部署的映像标签的变化以及我们在*步骤 5* 中增加的副本数量:
...
现在，您已经学习了如何使用复制集编辑、扩展和推出新版本的应用。
...
前面的命令将立即终止部署，并将应用从集群中删除。

# 它是如何工作的...

*创建部署*配方向您展示了如何使用 YAML 清单文件将您的 POD 和副本集的所需状态应用到部署控制器。
...
2.  使用`nginx`目录下的 YAML 文件配方，复制您在*部署工作负载中创建的`deployment-nginx.yaml`文件。该文件仍然使用`image: nginx:1.7.9`作为容器映像:*

```
$ cp deployment-nginx.yaml ./nginx/
```

3.  通过指定新的映像版本创建`kustomization.yaml`文件:
...
4.  通过运行以下命令，检查新版本是否已注入您的部署。在输出中，您将看到`image: nginx:1.16.0`而不是我们之前在`deployment-nginx.yaml`文件中使用的原始映像版本`nginx:1.7.9`:
...
6.  通过指定更新的映像版本创建新的`kustomization.yaml`文件:
...
现在，您已经学习了如何使用 Kustomize 编辑、放大和推出新版本的应用。
...
在*为开发和生产部署*配方创建基础中，我们在步骤 2 和步骤 6 之间在`base`目录下创建的资源代表应用/工作负载的上游存储库，我们在步骤 8 和步骤 10 之间在`overlay`目录下创建的定制是您控制并存储在存储库中的更改。
...
*   Kubernetes 背景白皮书中的声明性应用管理–强烈推荐阅读:[https://goo.gl/T66ZcD](https://goo.gl/T66ZcD)
...
在本节中，我们将向您展示如何在 Kubernetes 中使用 Helm 图表。Helm 是 Kubernetes 的包管理器，它帮助开发人员和 SREs 轻松地打包、配置和部署应用。

您将学习如何在集群上安装 Helm，并使用 Helm 管理第三方应用的生命周期。
...
同样，您可以从 Helm charts 稳定存储库中安装其他应用，或者为自定义图表添加自己的存储库。
...
让我们执行以下步骤，从 Helm 图表存储库中搜索您想要在 Kubernetes 上部署的应用:
...
2.  将来，您可能会发现特定版本的应用在您的环境中更稳定，或者在多个集群中保持安装相同。在这种情况下，您可以使用以下命令用您首选的图表版本更新图表版本:
...
让我们执行以下步骤来调用升级，并将您的应用状态恢复到以前的版本:
...
让我们执行以下步骤，从 Kubernetes 集群中删除使用 Helm 部署的应用:
...
2.  使用`--purge`参数删除版本。以下命令将从集群中完全删除应用:
...
Kubernetes 操作符是 Kubernetes 捆绑、部署和管理应用的另一种方法。运算符比像 Helm 这样的包管理器复杂一点。操作员帮助删除手动步骤、特定于应用的准备和部署后步骤，甚至自动化第二天的操作，例如为用户扩展或升级它们。

例如，应用的需求可能会根据其安装的平台进行不同的验证，或者可能需要更改其配置和与外部系统的交互。

在本节中，我们将基于两种不同的运算符框架为流行的有状态应用部署两种运算符，并了解它们提供了哪些功能。
...
在使用 KUDO 操作符安装应用之前，您需要安装 KUDO。我们将使用`brew`安装 KUDO，这是一个在 Linux 中使用的包管理器，用于在 Linux 上简单安装二进制文件；因此，如果您还没有安装 brew，您还需要安装:
...
现在您已经安装了 KUDO 控制器，使用 Kubernetes 操作符测试一些有状态运行的应用。
...
现在，您已经安装了 OLM，使用一个操作框架来测试一些有状态运行的应用。
...
*   按社区划分的 Kubernetes 运营商名单:[https://github.com/Operator-framework/awesome-Operators](https://github.com/operator-framework/awesome-operators)
*   使用红帽操作员软件开发工具包构建的 Kubernetes 操作员列表:[https://Operatorhub.io/](https://operatorhub.io/)
*   Kubernetes 通用声明符 ( **工藤**):[https://kudo.dev/](https://kudo.dev/)
...
*   替代卡夫卡运营商列表:
    ...
    *   Kubernetes 的另一个卡夫卡式的操作者:https://github.com/banzaicloud/kafka-Operator
...
詹金斯命令行界面`jx`与您首选的云提供商命令行界面一起用于协调 Kubernetes 集群的部署。Jenkins X 支持 Azure、AWS、 **GCP** (简称**谷歌云平台**)、IBM 云、Oracle 云、Minikube、Minishift 和 OpenShift 作为部署的提供者。对于这个食谱，我们将使用 GKE。有关其他供应商说明，请参见詹金斯 X 文档。
...
1.  使用带有`gke`参数的`jx`命令，使用 GKE 创建 Kubernetes 集群，如下所示:
...
1.  确认所有 POD 都在运行。`jx`命名空间中的所有单元都应该处于运行状态:
...
2.  选择您想要使用的集群。在我们的例子中，我们切换到使用詹金斯 X 创建的`gke_devopscookbook`集群:
...
创建詹金斯 X Kubernetes 集群的方法向您展示了如何为管道自动化和自动化 CI/CD 配置 Kubernetes 集群。
...
*   导入应用
*   升级詹金斯 X
*   删除詹金斯 X Kubernetes 集群

# 导入应用

让我们执行以下步骤，将现有应用导入到 Jenkins X 环境中:

1.  克隆或使用现有应用。作为示例，我们将创建`hello-world`示例的克隆:
...
让我们执行以下步骤来升级 Jenkins X 应用及其组件:

1.  首先，升级`jx` CLI。如果远程存储库中有新版本，此命令将升级应用:
...
现在，您已经学习了如何使用 Jenkins X 来创建集群。这些知识已经为您准备好了[第 3 章](03.html)、*构建 CI/CD 管道*，在这里您将继续使用环境并学习在 Jenkins X 中将应用作为管道导入
...
在我们使用 Helm 方法执行*安装 GitLab 中的命令之后，在步骤 2 中，Helm 图表假设我们有一个现有的默认存储类，并且它使用默认存储类来为有状态应用创建 PVC 和 PVs。*
...
现在，您已经学习了如何在 Kubernetes 上启动和运行 GitLab。在 *GitLab* 部分的[第 3 章](03.html)、*构建 CI* / *光盘管道*中将需要这些知识，在这里您将学习如何在 GitLab 中导入应用和创建管道。
