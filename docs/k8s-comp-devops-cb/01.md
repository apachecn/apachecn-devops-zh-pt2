...
2.  埃特娜
3.  在“创建群集”页面上，从子网列表中选择Kubernetes版本、角色名称、至少两个或更多可用性区域以及安全组。
4.  点击创建。
...
18.  监控和记录将在[第 8 章](08.html)、*库本内斯的可观测性和监控*、[第 10 章](10.html)、*库本内斯的记录*中说明。因此，这一步是可选的。检查`Install cloud monitoring plug-in on your ECS`和`Using Log Service`选项以启用监控和记录。
...
在第 5 步中，我们使用`kubectl drain`命令驱逐现有的吊舱，因为仅封锁不会对当前计划的吊舱产生影响。驱逐 API 将中断预算考虑在内。如果由所有者设置，中断预算会限制复制的应用因自愿中断而同时停机的盒数。如果不支持或设置这一点，退出应用编程接口将在宽限期后简单地删除节点上的 pod。
