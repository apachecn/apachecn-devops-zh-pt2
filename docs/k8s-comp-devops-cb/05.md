...
在本章中，我们将讨论在 Kubernetes 上使用流行的开源存储解决方案，以及如何保护应用的状态免受节点或应用故障的影响并共享相同的数据，或者当 pod 在不同的节点上重新计划时如何处理卷的重新附加。遵循本章中的方法后，您将获得在自我管理的基础架构或私有云中同时使用数据块和文件存储选项的技能。

在这一章中，我们将介绍以下食谱:

*   管理 Kubernetes 的亚马逊电子商务卷
*   管理 Kubernetes 的 GCE PD 卷
...
本节中的方法假设您已经根据[第 1 章](01.html)、*构建生产就绪型 Kubernetes 集群*中描述的推荐方法之一部署了一个功能性 Kubernetes 集群。

Kubernetes 的命令行工具`kubectl`将用于本节剩余的食谱，因为它是针对 Kubernetes 集群运行命令的主要命令行界面。我们还将使用`helm`来部署解决方案。

# 管理 Kubernetes 的亚马逊电子商务卷

亚马逊弹性块存储(亚马逊 EBS)为亚马逊 EC2 实例提供持久的块级存储卷，这些实例在 AWS 中由`kops`创建的 Kubernetes 集群和亚马逊 EKS 集群中使用。在本节中，我们将为您在 AWS 中运行的 Kubernetes 中的应用创建存储类。您将学习如何在 Kubernetes 中创建存储类资源，您将了解随着 EBS 卷类型性能的变化而变化的可用参数，并使用新的**容器存储接口** ( **CSI** )来使用 EBS 卷。
...
存储类是动态资源调配的基础。正如您在我们的示例中看到的，您的集群中可能有多个存储类。理想情况下，创建存储类时应该考虑应用的需求，因为某些应用需要更快的卷，而其他应用可能会利用 Rook 和 OpenEBS 等解决方案提供的多可用性区域复制，我们将在本章稍后部分进行部署。

# 更改默认存储类别

动态存储资源调配是扩展应用的关键部分。当存储类不是由**持久卷声明** ( **聚氯乙烯**指定时，Kubernetes 使用默认选项。让我们执行以下步骤，将首选存储类设置为默认存储类:
...
作为创建物理卷和动态创建卷的替代方法，您也可以手动创建一个卷，并通过执行以下步骤将其作为永久卷直接附加到您的应用:
...
2.  使用您在*步骤 1:* 中创建的 EBS `volumeID`部署测试应用
...
作为状态资源集的一部分，`volumeClaimTemplates`可以使用由您选择的`PersistentVolume`资源调配者调配的`PersistentVolumes`来提供持久存储。在本食谱中，我们将使用 StorageClass 为您的应用动态创建 PVs。让我们从以下步骤开始:

1.  在应用部署清单的`volumeClaimTemplates`部分下添加`aws-gp2`存储类行，类似于以下示例:
...
4.  验证是否已经创建了 POD。在这个食谱中，我们的例子有三个副本的状态集。因此，您应该会看到三个副本正在运行，类似于以下输出:
...
我们已经成功删除了与作为 Redis StatefulSet 资源的一部分创建的应用相关的所有存储资源。
...
**谷歌云平台**(**【GCP】**)提供持久的块级存储卷，供**谷歌 Kubernetes 引擎** ( **GKE** )实例使用。在本节中，我们将为您在 GKE 运行的 Kubernetes 中的应用创建存储类。您将学习在 Kubernetes 中创建一个 StorageClass 资源，您将了解随着 GCP PD 卷类型性能的变化而变化的可用参数，并使用新的 CSI。
...
GKE 附带了一个名为`standard`的默认存储类。您的集群中可能有多个存储类。理想情况下，创建存储类时应考虑应用的要求，因为某些应用需要更快的卷，而其他应用可能会利用其他解决方案提供的多可用性区域复制。

# 更改默认存储类别

动态存储资源调配是扩展应用的关键部分。当存储类不是由 PVC 指定时，Kubernetes 使用默认选项。让我们执行以下步骤，将首选存储类设置为默认存储类:
...
作为创建物理卷和动态创建卷的替代方法，您也可以手动创建一个卷，并通过观察以下步骤将其作为永久卷直接附加到您的应用:
...
4.  使用您在*步骤 1:* 中创建的`volumeMounts`名称`gce-disk-1`部署测试应用
...
作为状态资源集的一部分，`volumeClaimTemplates`可以使用由您选择的`PersistentVolume`资源调配者调配的`PersistentVolumes`来提供持久存储。在本食谱中，我们将使用 StorageClass 为您的应用动态创建 PVs:

1.  在应用部署清单的`volumeClaimTemplates`部分下添加`gce-pd`存储类行，类似于以下示例:
...
现在，您知道如何动态创建 GCE PD 持久卷，作为应用部署的一部分。
...
GCP 计算 PD CSI 驱动程序提供了 Kubernetes CSI 接口，允许 GKE 集群简单地管理持久卷的 GKE 卷的生命周期。在本食谱中，我们将通过观察以下步骤来了解安装 GCP 计算 PD CSI 驱动程序所需的步骤:
...
在 GKE 集群上构建高可用性有状态工作负载时，建议使用此选项。
...
Azure Cloud 提供持久的块级存储卷，供 **Azure Kubernetes 引擎** ( **AKS** )使用。在本节中，我们将为您在 AKS 中运行的 Kubernetes 中的应用创建存储类。您将学习如何在 Kubernetes 中创建存储类资源，了解随着 Azure Disk 卷类型性能的变化而变化的可用参数，还将学习如何使用新的 CSI。
...
作为状态资源集的一部分，`volumeClaimTemplates`可以使用由您选择的`PersistentVolume`资源调配者调配的`PersistentVolumes`来提供持久存储。在本食谱中，我们将使用 Azure 存储类为您的应用动态创建 PVs:

1.  在应用部署清单的`volumeClaimTemplates`部分下添加`azure-zrs`存储类行，类似于以下示例:
...
现在，您知道了如何在 AKS 集群上动态创建持久卷作为应用部署的一部分。
...
我们现在已经成功删除了与作为 Redis StatefulSet 资源的一部分创建的应用相关的所有存储资源。
...
Rook 是 Kubernetes 的一个云原生开源存储编排器。Rook 在 Kubernetes 中提供了自我管理、自我扩展和自我修复的分布式存储系统。在本节中，我们将为您在 Kubernetes 中的应用使用 Rook 存储编排器创建多个存储提供商。您将学习为需要持久存储的有状态应用创建 Ceph 提供程序。
...
6.  现在列出 POD，并确保您的工作负载、OpenEBS 控制器和副本都处于运行状态:
...
现在，您知道如何在 Kubernetes 上为您的有状态应用配置高可用性的云原生存储。
...
您现在已经成功地部署了一个有状态的应用，该应用部署在 OpenEBS cStor 存储引擎上。
...
尽管这不是性能最好的解决方案，但 NFS 仍用于需要多节点写访问的云原生应用。在本节中，我们将为这种类型的应用创建一个基于 NFS 的持久存储。您将学习如何使用 OpenEBS 和 Rook 为需要在 Kubernetes 上共享存储的有状态工作负载创建可访问的持久卷。
...
NFS 在 Kubernetes 环境中使用，是因为它对需要同时访问相同数据的应用的`ReadWriteMany`能力。在本食谱中，我们将执行以下步骤来动态创建基于 NFS 的持久卷:
...
2.  现在，您可以使用`rook-nfs-share1`存储类为需要`ReadWriteMany`访问的应用创建 PVC:
...
2.  现在，您可以使用`openebs-nfs`存储类为需要`ReadWriteMany`访问的应用创建 PVC:
...
您已经部署了一个应用，但是 pods 和持久卷声明都处于挂起状态，类似于以下内容:
...
5.  使用`kubectl apply -f <deployment.yaml>`重新部署应用。
6.  确认聚氯乙烯状态为`Bound`:
