...
*   管理Kubernetes的亚马逊电子商务卷
*   管理Kubernetes的 GCE PD 卷
...
本节中的方法假设您已经根据[第 1 章](01.html)、*构建生产就绪型Kubernetes集群*中描述的推荐方法之一部署了一个功能性Kubernetes集群。
...
动态存储资源调配是扩展应用的关键部分。当存储类不是由**持久卷声明** ( **聚氯乙烯**指定时，Kubernetes使用默认选项。让我们执行以下步骤，将首选存储类设置为默认存储类:
...
4.  验证是否已经创建了POD。在这个食谱中，我们的例子有三个副本的状态集。因此，您应该会看到三个副本正在运行，类似于以下输出:
...
6.  现在列出POD，并确保您的工作负载、OpenEBS 控制器和副本都处于运行状态:
