...
![](assets/db5c9baa-10e6-4180-a8f6-3ab0ae36f8ed.png)
...
![](assets/90d34cbf-d24d-4ea0-ac1e-75d3c6141940.png)
...
![](assets/83c59446-7856-4c66-ae92-a3ae33b73287.png)
...
![](assets/f4a042ca-aa04-4b2c-89ff-b00f636534b1.png)

3.  打开`backups`目录，找到您的 Velero 备份列表:

![](assets/4b9f7c66-bcf2-4e3b-b5f6-055ad26d7170.png)

4.  单击备份名称以访问备份内容:

![](assets/5411859b-4bac-4063-b0bd-0f040fbaeb81.png)
...
此菜谱向您展示了如何创建灾难恢复备份，如何从 S3 目标恢复应用程序及其数据，以及如何在 Kubernetes 上创建计划备份作业。
...
![](assets/de09bcd5-42ec-451c-9d73-01356b603734.png)
...
![](assets/097c780e-395e-42fc-8e2d-9948e306fd42.png)

6.  在`backup-example`命名空间中，单击创建策略:

![](assets/d5849d36-9265-4abb-a2c0-44059275c645.png)

7.  输入名称并选择快照操作:

![](assets/fe69e77a-1598-4951-b286-35cfeeb7473c.png)

8.  选择“每日”作为操作频率:

![](assets/95c89eb6-d2e4-4c58-b543-63be64c1e915.png)

9.  单击创建策略:

![](assets/39be9559-7db2-4a29-887b-5288c802cb4a.png)

通过执行这些步骤，您将使用策略创建了第一个备份，并为以下备份作业创建了时间表。

# 恢复应用程序

让我们执行以下步骤，从现有备份中恢复应用程序:

1.  在应用程序下，从兼容应用程序列表中，单击`backup-example`旁边的箭头图标，并选择恢复应用程序。如果应用程序已被删除，则需要选择“已删除”选项:

![](assets/b47e1fbf-8a12-4b9d-8ff6-36fba428ad66.png)

2.  选择要恢复到的还原点:

![](assets/957f1236-1069-40c6-8af3-41705472f2e6.png)

3.  选择`backup-example`并点击恢复:

![](assets/78eea102-ca73-4f21-a3d6-ce70a4591fbc.png)

4.  确认您想要恢复:

![](assets/d49b6866-0308-48db-ba93-2fa952603128.png)
...
![](assets/6d359dd3-d22f-4823-9b8e-6bcb4cdebab9.png)

2.  要创建目标配置文件，请选择导出，选中启用数据可移植性框，选择亚马逊 S3，然后输入您的用户凭据。
3.  单击验证并保存:

![](assets/761743f3-17c8-4586-b87d-869389e0e06d.png)
...
![](assets/f7e1cd15-29d2-4089-b051-ca0c3f48f4fe.png)

3.  选择您的导出配置文件，然后单击导出:

![](assets/bf7e68f0-7203-432d-bc0a-cc34f16d5b97.png)

4.  确认恢复。
5.  将文本块复制到剪贴板:

![](assets/fab2c2a8-545a-4c73-a705-3353c4175070.png)
...
![](assets/db92840c-0a7d-44dc-aed6-ee7d519b15aa.png)

2.  要创建目的地配置文件，请选择导入，选择亚马逊 S3，然后输入您的用户凭据。
3.  使用您为源群集上的导出配置文件创建的存储桶名称。
4.  单击验证并保存:

![](assets/c6f9b7bc-317c-443e-8a02-5b58491bc704.png)
...
![](assets/6e066a04-e429-4104-94d3-ebc9265c3794.png)

2.  选择导入并选中导入后恢复框。
3.  选择每日作为操作频率，并从*导出还原点*配方粘贴配置数据文本块。
4.  选择您在*创建导入配置文件*配方中创建的导入配置文件:

![](assets/7dfa8b9f-2ac5-498f-8387-edfd89bb2c6d.png)
...
![](assets/07158394-ffc6-47f1-9f26-c92b4c9a9624.png)

2.  单击连接您的集群按钮:

![](assets/4496791f-a9fc-410e-b5be-a8e99d898b09.png)

3.  说出你的项目。这里，我们用了`GKECluster`这个名字:

![](assets/d7cd5672-205f-4923-9009-0f250adac284.png)

4.  选择您的 Kubernetes 集群位置。这里，我们在`GKE`上使用了一个集群:

![](assets/20074fbf-440b-48a4-afee-2cbd4249a8f7.png)

5.  在第一个集群上复制并执行命令:

![](assets/f84e21af-b8f1-456c-ad1a-c1a8d27aba41.png)

6.  从左侧菜单中，单击集群:

![](assets/3608f3d9-2224-4cc0-8dae-3986f4836acf.png)

7.  在集群视图中，单击连接新集群，并对第二个集群重复*步骤 4* 和*步骤 5* :

![](assets/01135d95-ff12-4102-8f56-45989c94c1bc.png)
...
![](assets/dd6cfb7b-bd0c-4634-9a11-c21a4197e49d.png)

2.  在源群集的“概述”页面上，单击要迁移的工作负载。在本食谱中，我们将迁移 MinIO 工作负载:

![](assets/3eddf2df-24f8-40b2-8e0b-1ba51d578bfc.png)

3.  在“应用程序”视图中，选择“数据助理”选项卡，然后单击“新建计划”按钮:

![](assets/1fc3c3e5-0cf6-4529-ab9c-2b4f3023a043.png)

4.  在“新计划”视图中，选择 AWS 作为 S3 提供商，并选择您的凭据和地区。最后，选择备份间隔为每日，然后单击立即计划按钮创建备份。或者，您也可以使用 GCP 或 MinIO 作为 S3 目标:

![](assets/f89e629a-5034-44a4-b357-0e52e4f0e8b2.png)

5.  从左侧菜单中，选择“数据助理”，然后单击您创建的时间表旁边的“恢复”按钮:

![](assets/88c21f40-fc35-4cf0-b44e-8748dc5f395e.png)

6.  从托管群集列表中选择您的目标群集，然后单击开始恢复:

![](assets/1eb9800e-d9e3-462d-88af-1a5ef01d978c.png)
