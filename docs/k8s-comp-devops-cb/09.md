...
2.  使用以下规则创建角色。该角色基本上允许在我们在*步骤 1 中创建的`secureapp`命名空间中的`deployer`角色的部署、副本集和POD上执行所有操作。*请注意，授予的任何权限都只是附加的，没有拒绝规则:
...
9.  用前一个命令的输出替换 pod 名称，并查看 pod 日志以检索`kube-bench`结果。在我们的示例中，POD名称为`kube-bench-7lxzn`:
...
5.  使用`config/`目录中的配置文件创建配置图。稍后，DaemonSet 将使用配置映射为 Falco POD提供配置:
...
3.  让我们测试一下法尔科是如何工作的，方法是在其中一个法尔科POD中安装一个 bash shell，然后查看日志。列出法尔科豆荚:
...
在第 6 步中，当我们创建 DaemonSet 时，所有默认规则都是通过 ConfigMap 中的`falco_rules.yaml`文件提供的。这些放在POD内部的`/etc/falco/falco_rules.yaml`中，而本地规则文件`falco_rules.local.yaml,`可以在`/etc/falco/falco_rules.local.yaml`中找到。
...
5.  验证POD的状态。您会注意到POD还没有准备好，因为准备探测器需要首先初始化 Vault:
...
9.  验证POD的状态。您将看到就绪探测器已经过验证，并且POD已经就绪:
...
这个食谱将向你展示如何在Kubernetes使用金库，并从金库中检索秘密。
