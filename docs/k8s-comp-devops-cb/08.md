...
使用 Kubernetes 仪表板配方的*监控指标需要 Kubernetes 仪表板 v2.0.0 或更高版本才能运行。如果您想向仪表板添加度量功能，请按照[第 1 章](01.html)、*构建生产就绪型Kubernetes集群*中的*部署Kubernetes仪表板*配方中的说明，确保安装了Kubernetes仪表板。*
...
1.  确认`kubernetes-metrics-scraper`POD正在运行。如果没有，请按照*中的说明安装Kubernetes仪表板*在[章节](01.html)中部署Kubernetes仪表板[r](01.html)[1](01.html)*构建生产就绪Kubernetes集群*:
...
确保您已经准备好亚马逊 EKS Kubernetes集群并配置`kubectl`来管理集群资源。如果您还没有这样的集群，您可以按照[第 1 章](01.html)、*构建生产就绪型Kubernetes集群*、在亚马逊网络服务上配置Kubernetes集群一节中的说明进行操作。
...
在本食谱中，我们将使云观察代理能够从我们的 EKS Kubernetes集群中收集集群指标:
...
确保您已经准备好 GKE 集群并配置`kubectl`来管理集群资源。如果您还没有，您可以按照*在谷歌云平台上配置Kubernetes集群*食谱中[第 1 章](01.html)、*构建生产就绪Kubernetes集群*中的说明进行操作。
...
3.  在集群配置页面上，确保禁用了传统堆栈河日志记录和传统堆栈河监控，并将堆栈河Kubernetes引擎监控选项设置为启用:
...
4.  在基础架构选项卡上，单击集群名称旁边的展开图标。Stackdriver 将使用单个工作节点展开列表。在“就绪”列中，您可以看到每个节点中已部署且处于就绪状态的POD数量。在“CPU 利用率”列中，左侧的值显示可用 CPU 总数，右侧的值显示当前利用率百分比。同样，在“内存利用率”列中，左侧的值显示总可用内存(GiB)，右侧的值显示当前利用率百分比:
...
确保您已经准备好 AKS 集群并配置`kubectl`来管理集群资源。如果您还没有，您可以按照*在谷歌云平台上配置Kubernetes集群*食谱中[第 1 章](01.html)、*构建生产就绪Kubernetes集群*中的说明进行操作。
