...
用户帐户是为人类(集群管理员或开发人员)准备的，他们通常通过 kubectl 或编程方式从外部操作 Kubernetes。最终用户不应该拥有 Kubernetes 用户帐户，而应该只有应用级用户帐户。这与Kubernetes无关。请记住，Kubernetes 为您管理您的容器——它不知道内部发生了什么，也不知道您的应用实际在做什么。
...
在授予我们的服务帐户使用 RBAC 的超级权力之前，让我们回顾一下Kubernetes是如何管理秘密的。默认情况下，Kubernetes 在 etcd 中存储秘密。将 etcd 与第三方解决方案集成是可能的，但在本节中，我们将重点讨论普通的 Kubernetes。秘密应该在静态和传输中加密，etcd 从版本 3 开始就支持这一点。

现在我们已经了解了在 Kubernetes 中帐户是如何工作的，让我们看看如何管理秘密。

# 与Kubernetes一起管理秘密

在授予我们的服务帐户使用 RBAC 的超级权力之前，让我们回顾一下Kubernetes是如何管理秘密的。默认情况下，Kubernetes 将秘密存储在 etcd([https://coreos.com/etcd/](https://coreos.com/etcd/))中。Kubernetes 可以管理不同类型的秘密。让我们看看各种秘密类型，然后创建我们自己的秘密，并将它们传递给容器。最后，我们将一起建造一个安全的POD。
...
最后一类秘密是`Opaque`，也是最有趣的一类秘密。你把你的敏感信息储存在不透明的秘密中，Kubernetes不接触。它只是为您提供了一个强大而安全的秘密存储库，以及一个用于创建、读取和更新这些秘密的应用编程接口。您可以通过多种方式创建不透明的机密，例如:
...
2.  现在，我们将把它连接到我们的POD上，并设置我们之前创建的`imagePullSecret`。这里发生了很多事情。我附加了一个定制服务帐户，创建了一个引用`generic-secrets2`秘密的秘密卷，然后一个将它装入`/etc/generic-secrets2`的卷安装；最后，我给`private-dockerhub`设定了`imagePullSecrets`的秘密:
...
RBAC 是一个用来管理对Kubernetes资源的访问的机制。从 Kubernetes 1.8 开始，RBAC 被认为是稳定的。用`--authorization-mode=RBAC`启动 API 服务器启用。当向 API 服务器发出请求时，RBAC 的工作方式如下:
...
在这一点上，`link-manager`和`social-graph-manager`有一个共享的秘密，我们可以安装到各自的POD。这是`link-manager`部署中的POD规格，它将秘密从一个卷装入`/etc/delinkcious`。这个秘密会以`mutual-auth.yaml`文件的形式出现:
...
Kubernetes 有一个名为`AlwaysPullImages`的准入控制器，它将每个POD的`ImagePullPolicy`设置为`AlwaysPullImages`。这可以防止所有问题，但代价是提取映像，即使它们存在并且您有权使用它们。您可以通过将该准入控制器添加到通过`--enable-admission-controllers`标志传递给`kube-apiserver`的已启用准入控制器列表中来打开该准入控制器。
...
名称空间是一个非常直观但强大的概念。然而，它们本身并不阻止同一个集群中的POD相互通信。在 Kubernetes 中，集群中的所有豆荚共享相同的平面网络地址空间。这是 Kubernetes 网络模块的最大简化之一。您的豆荚可以在相同的节点上，也可以在不同的节点上，这并不重要。
...
虽然每个容器都指定了资源，但是当容器有多个容器时，考虑整个容器的总资源请求(所有容器请求的总和)是很重要的。其原因是POD总是作为一个单元进行调度。如果您有一个包含 10 个容器的容器，每个容器都需要 2 Gib 的内存，那么这意味着您的容器需要一个具有 20 Gib 可用内存的节点。
