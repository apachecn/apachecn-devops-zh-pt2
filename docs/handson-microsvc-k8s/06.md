...
Kubernetes 有一个名为`AlwaysPullImages`的准入控制器，它将每个吊舱的`ImagePullPolicy`设置为`AlwaysPullImages`。这可以防止所有问题，但代价是提取映像，即使它们存在并且您有权使用它们。您可以通过将该准入控制器添加到通过`--enable-admission-controllers`标志传递给`kube-apiserver`的已启用准入控制器列表中来打开该准入控制器。
