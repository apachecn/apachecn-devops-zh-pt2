...
在授予我们的服务帐户使用 RBAC 的超级权力之前，让我们回顾一下库本内斯是如何管理秘密的。默认情况下，Kubernetes 将秘密存储在 etcd([https://coreos.com/etcd/](https://coreos.com/etcd/))中。Kubernetes 可以管理不同类型的秘密。让我们看看各种秘密类型，然后创建我们自己的秘密，并将它们传递给容器。最后，我们将一起建造一个安全的POD。
