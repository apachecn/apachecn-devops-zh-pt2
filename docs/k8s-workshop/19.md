...
前两章奠定了容器化和库本内特的基础。此后，我们学习了 kubectl 的基础知识 Kubernetes 指挥中心。在*第 04 章如何与 Kubernetes (API Server)* 中，我们看了 Kubernetes 和其他 HTTP 客户端如何与 Kubernetes API server 进行通信。我们通过在本章末尾创建一个部署来巩固我们的学习。
