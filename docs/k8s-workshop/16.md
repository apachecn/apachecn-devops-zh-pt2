...
*   **变异准入控制器**允许你在申请到Kubernetes平台之前修改申请。`LimitRanger`就是一个这样的例子，如果POD本身没有定义`defaultRequests`，它将把`defaultRequests`应用到POD。
...
1.  一个简单的 HTTPS 服务器(网络挂钩)被设置在一个POD中，以接受开机自检呼叫。请注意，呼叫必须通过 SSL 进行。
...
10.  You might need to wait a bit and check whether the webhook Pods have been created. Keep checking the status of the Pods:
    ...
    请注意，`-w`标志持续观察 Pods。当所有的POD都准备好了，你可以结束手表。
...
正如你所看到的，这个POD没有任何标签，但我们能够创建它。这是因为我们的验证 webhook 没有关注`webhooks-demo`名称空间。
