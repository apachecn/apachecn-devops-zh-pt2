...
![Figure 16.1: Stages of an API request for creating an object ](image/B14870_16_01.jpg)
...
![Figure 16.2: Starting up Minikube with all of the required plugins  to run admission controllers ](image/B14870_16_02.jpg)
...
![Figure 16.3: Process flow for the mutating admission webhook ](image/B14870_16_03.jpg)
...
3.  Check the contents of the ConfigMap using the following command:
    ...
    ![Figure 16.4: Getting the contents of the ConfigMap in YAML format ](image/B14870_16_04.jpg)
...
6.  Now, let's see whether the patch has been applied. Use the following command:
    ...
    ![Figure 16.5: Checking the modified annotations on our ConfigMap ](image/B14870_16_05.jpg)
...
1.  Create a **Certificate Authority** (**CA**) for a self-signed certificate. This CA will be later used to create trust between the Kubernetes and our webhook server for the HTTPS call:
    ...
    ![Figure 16.6: Generating a self-signed certificate ](image/B14870_16_06.jpg)
...
2.  Create the private key for the SSL call:
    ...
    ![Figure 16.7: Creating the private key for the SSL call ](image/B14870_16_07.jpg)
...
![Figure 16.8: Expected output of Activity 16.01 ](image/B14870_16_08.jpg)
...
![Figure 16.9: Expected output of the Activity 16.02 ](image/B14870_16_09.jpg)
...
2.  Now, delete the preexisting validating webhook defined in *Activity 16.02*, *Creating a Validating Webhook that Checks for a Label in a Pod*:
    ...
    ![Figure 16.10: Deleting the existing validating webhook ](image/B14870_16_10.jpg)
...
5.  Create a new CA bundle and a private/public key pair to be used in this webhook. Generate a self-signed certificate using this command:
    ...
    ![Figure 16.11: Generating a self-signed certificate ](image/B14870_16_11.jpg)
...
6.  Generate a private/public key pair and sign it with the CA certificate using the following two commands, one after the other:
    ...
    ![Figure 16.12: Signing a private/public key pair with our certificate ](image/B14870_16_12.jpg)
...
10.  You might need to wait a bit and check whether the webhook Pods have been created. Keep checking the status of the Pods:
    ...
    ![Figure 16.13: Checking whether our webhook is online ](image/B14870_16_13.jpg)
...
13.  Create a Base64-encoded version of the CA certificate. Use the following commands, one after the other:
    ...
    ![Figure 16.14: Contents of the Base64-encoded CA certificate ](image/B14870_16_14.jpg)
...
14.  Use the following two commands to clean up the blank lines from our CA certificate and add the contents to a new file:
    ...
    ![Figure 16.15: Base64-encoded CA certificate with the line breaks removed ](image/B14870_16_15.jpg)
...
16.  Create the webhook, as defined in the previous step. Make sure that you replace the `caBundle` field with the certificates created in the earlier steps:
    ...
    ![Figure 16.16: Creating the ValidatingWebhookConfiguration ](image/B14870_16_16.jpg)
...
20.  Based on the definition from the previous step, create the Pod in the `webhooks` namespace:
    ...
    ![Figure 16.17: Pod rejected due to the absence of the required label ](image/B14870_16_17.jpg)
...
22.  Let's describe the Pod to get more details:
    ...
    ![Figure 16.18: Checking the specification of our Pod ](image/B14870_16_18.jpg)
