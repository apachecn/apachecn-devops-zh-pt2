...
黑客实施的许多攻击都是为了完成上述一项或多项任务，通常会危及我们数据的机密性、完整性和可用性。考虑到这一点，我们可以使用一个心理模型，来思考在现代云原生应用堆栈的各个部分中，我们的系统可能面临哪些威胁。这种心理模型被称为“云原生安全的 4Cs”，我们将使用它来组织我们对Kubernetes安全原语的探索。理想情况下，通过利用所有这些原语，这应该会让您对应用抵抗类似 STRIDE 的攻击有很好的信心，特别是在 Kubernetes 的环境中。
...
*   **Kubernetes RBAC** :这是Kubernetes的主要政策引擎。它定义了一个角色和权限系统，以及如何向这些角色授予权限。
...
在我们深入 RBAC 之前，回想一下*第 4 章*、*如何与Kubernetes(API 服务器)*通信，Kubernetes如何向 API 授权请求。我们了解到有三个阶段——身份验证、授权和授权控制。我们将在*第 16 章*、*Kubernetes入场控制员*中了解更多入场控制员的信息。
...
*   `verbs`:这些描述了我们允许哪些Kubernetes人的行为。常用动词的一些例子包括`get`、`list`、`watch`、`create`、`update`和`delete`。还有更多，但对于大多数用例来说，这些通常已经足够好了。关于这方面的复习，请参考*第四章**如何与Kubernetes(API 服务器)*的*Kubernetes API* 部分。
...
## 练习 13.01:创建Kubernetes·RBAC 集群角色
...
如果这是一个用户(或黑客)试图使用任何不允许他们使用的命令，我们会成功阻止它。在 Kubernetes 文档网站上有许多实用的 RBAC 示例。在Kubernetes谈论 RBAC 的所有设计模式超出了本章的范围。我们所能说的是:只要有可能，您应该实践“最小特权原则”来限制对 Kubernetes API 服务器的不必要的访问。也就是说，每个人都应该获得完成工作所需的最低访问级别；不是每个人都需要成为集群管理员。
...
*   `volumes`:这允许我们允许不同类型的体积可以安装到那个POD上，例如`configmap`或`persistentVolumeClaim`。在这个示例规范中，我们指定了`*`(星号)，这意味着所有类型的卷都允许被这个 Pod 中的进程使用。
...
1.  确保您拥有集群基础设施和练习 13.01“创建Kubernetes·RBAC 集群角色”中的所有对象。
