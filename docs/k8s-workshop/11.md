...
对于较小的部署和简单的操作，从**容器网络接口** ( **CNI** )的角度来看，kubenet 远远超过了这些集群的需求。但是，它并不适用于所有工作负载和网络拓扑。因此，Kubernetes 为几种不同的中枢神经系统提供支持。当从高可用性的角度考虑容器网络接口时，您会想要尽可能高性能和稳定的选项。详细讨论 CNI 的每一个产品已经超出了介绍库本内特的范围。
