...
*第 2 章*、*Kubernetes概述*，为您提供了第一次对Kubernetes的实际操作介绍，并提供了Kubernetes架构的概述。
...
*第 17 章*、*Kubernetes*中的高级调度描述了调度器如何将POD放置在Kubernetes集群上。您将使用高级功能来影响POD的调度器放置决策。

*第 18 章**无停机升级集群*教你如何将 Kubernetes 平台升级到更新版本，而不会让你的平台或应用出现任何停机。

*第 19 章*、*Kubernetes*中的自定义资源定义，向您展示了扩展Kubernetes提供的功能的主要方式之一。您将看到自定义资源如何允许您在集群上实现特定于您自己的域的概念。
