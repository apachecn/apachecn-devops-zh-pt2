...
请注意，关联性和反关联性规则是基于节点上的标签定义的。如果节点上的标签发生更改，则可能不再应用节点关联规则。在这种情况下，正在运行的 Pods 将继续在节点上运行。如果一个POD重新启动，或者如果它死亡，一个新的POD被创建，Kubernetes认为这是一个新的POD。在这种情况下，如果节点标签已被修改，调度程序可能不会将 Pod 放在同一节点上。这是您在修改节点标签时需要注意的事情。让我们在下面的练习中为 Pod 实现这些规则。
...
POD将被移除或进入`Terminating`(标记为移除)状态。几秒钟后，Kubernetes将移除豆荚。
