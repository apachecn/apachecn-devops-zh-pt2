...
![Figure 15.1: An overview of the monitoring pipeline that  we will implement in this chapter ](image/B14870_15_01.jpg)
...
2.  To set up the Kubernetes Metrics server in our cluster, we need to run the following in sequence:
    ...
    ![Figure 15.2: Deploying all the objects required for the Metrics server ](image/B14870_15_02.jpg)
...
3.  To test this, let's run the following command:
    ...
    ![Figure 15.3: Response from the Kubernetes Metrics server ](image/B14870_15_03.jpg)
...
4.  Now, we have metric data being aggregated. Let's start scraping and visualizing with Prometheus and Grafana. For this, we will install Prometheus and Grafana using Helm. Run the following command:
    ...
    ![Figure 15.4: Installing the Helm chart for Prometheus ](image/B14870_15_04.jpg)
...
5.  Now, let's install Grafana in a similar fashion:
    ...
    ![Figure 15.5: Installing the Helm chart for Grafana ](image/B14870_15_05.jpg)
...
8.  Now, open your browser and visit `http://localhost:3000` to access Grafana. You should see the following landing page:
    ...
    ![Figure 15.6: The log-in page for the Grafana dashboard ](image/B14870_15_06.jpg)
...
9.  After a successful login, you should see this page:
    ...
    ![Figure 15.7: The Grafana Home dashboard ](image/B14870_15_07.jpg)
...
10.  Now, let's create a dashboard for Kubernetes metrics. To do so, we need to set up Prometheus as a data source for Grafana. On the left sidebar, click on `Configuration` and then on `Data Sources`:
    ...
    ![Figure 15.8: Selecting Data Sources from the Configuration menu ](image/B14870_15_08.jpg)

    图 15.8:从配置菜单中选择数据源

11.  You will see this page:

    ![Figure 15.9: The Add data source option ](image/B14870_15_09.jpg)
...
12.  You should see this page with several database options. Prometheus should be on top. Click on that:
    ...
    ![Figure 15.10: Choosing Prometheus as our data source for the Grafana dashboard ](image/B14870_15_10.jpg)
...
13.  Open a new terminal window and run the following command:
    ...
    ![Figure 15.11: Getting the list of all services ](image/B14870_15_11.jpg)
...
14.  After *step 12*, you will have arrived at the screen shown in the following screenshot:
    ...
    ![Figure 15.12: Entering the address of our Prometheus service in Grafana ](image/B14870_15_12.jpg)
...
15.  Now, let's set up the dashboard. Back on the Grafana home page (`http://localhost:3000`), click on the `+` symbol on the left sidebar, and then click on `Import`, as shown here:
    ...
    ![Figure 15.13: Navigating to import Dashboard option ](image/B14870_15_13.jpg)

    图 15.13:导航到导入仪表板选项

16.  On the next page, you should see the `Grafana.com Dashboard` field, as shown here:

    ![Figure 15.14: Entering the source to import the dashboard from ](image/B14870_15_14.jpg)
...
17.  The previous step should lead you to this screen:
    ...
    ![Figure 15.15: Setting Prometheus as the data source for the imported dashboard ](image/B14870_15_15.jpg)
...
18.  The result should look like this:
    ...
    ![Figure 15.16: The Grafana dashboard to monitor our cluster ](image/B14870_15_16.jpg)
...
1.  Now, let's deploy the `ingress-nginx` setup by running the following commands one after the other:
    ...
    ![Figure 15.17: Deploying the nginx Ingress controller ](image/B14870_15_17.jpg)
...
3.  Let's deploy this application by running the following command:
    ...
    ![Figure 15.18: Deploying our application ](image/B14870_15_18.jpg)
...
4.  With that, we are ready to load test. Before we begin, let's check on the number of Pods in our deployment:
    ...
    ![Figure 15.19: Getting details about our HPA ](image/B14870_15_19.jpg)
...
5.  Run the following command to first get your Ingress endpoint:
    ...
    ![Figure 15.20: Checking our Ingress endpoint  ](image/B14870_15_20.jpg)
...
6.  In another terminal session, run a `wrk` load test using the following command:
    ...
    ![Figure 15.21: Running a load test to our Ingress endpoint ](image/B14870_15_21.jpg)
...
7.  In another session, let's keep an eye on the pods for our application:
    ...
    ![Figure 15.22: Watching pods backing our application ](image/B14870_15_22.jpg)

    图 15.22:观察支持我们应用程序的吊舱
...
9.  Now, access the dashboard on your browser at `localhost:3000`:
    ...
    ![Figure 15.23: Observing our cluster in the Grafana dashboard ](image/B14870_15_23.jpg)
...
![Figure 15.24: Cluster with nodes at full capacity ](image/B14870_15_24.jpg)
...
![Figure 15.25: Additional node provisioned to run the additional pods ](image/B14870_15_25.jpg)
...
2.  Now, let's run the following command to create an AWS IAM policy:
    ...
    ![Figure 15.26: Creating an AWS IAM policy ](image/B14870_15_26.jpg)
...
3.  Now, we need to create an IAM user and then attach a policy to it. First, let's create the user:
    ...
    ![Figure 15.27: Creating an IAM user to use our policy ](image/B14870_15_27.jpg)
...
5.  Now, we need the secret access key for this IAM user. Run the following command:
    ...
    ![Figure 15.28: Fetching the secret access key for the created IAM user ](image/B14870_15_28.jpg)
...
10.  Now, apply this file by running the following:
    ...
    ![Figure 15.29: Deploying our ClusterAutoscaler ](image/B14870_15_29.jpg)
...
11.  Note that we need to now modify our ASG in AWS to allow for a scale-up; otherwise, the ClusterAutoscaler will not attempt to add any nodes. To do this, we have provided a modified `main.tf` file that has only one line changed: `max_size = 5` (*line 299*). This will allow the cluster to add up a maximum of five EC2 nodes to itself.
    ...
    ![Figure 15.30: Downloading the modified Terraform file ](image/B14870_15_30.jpg)
...
12.  Now, apply the modifications to the Terraform file:
    ...
    ![Figure 15.31: Applying our Terraform modifications ](image/B14870_15_31.jpg)
...
![Figure 15.32: Increase in the number of nodes observed  by describing the AWS scaling group ](image/B14870_15_32.jpg)

图 15.32:通过描述 AWS 缩放组观察到的节点数量的增加

您还应该能够在您的 Grafana 仪表板中观察到同样的情况:

![Figure 15.33: Increase in the number of nodes observed in the Grafana dashboard ](image/B14870_15_33.jpg)
