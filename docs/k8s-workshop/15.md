...
然后，在本章中，我们研究了如何使用数据扩展应用运行时的管理，特别是在介绍普罗米修斯、格拉夫纳和库本内斯度量服务器时。然后，我们利用这些信息来利用 HPA 和 ClusterAutoscaler，这样我们就可以放心了，我们的群集总是有适当的规模，并且随时可以自动响应需求高峰，而不必为过度配置的硬件付费。
