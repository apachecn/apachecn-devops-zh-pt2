...
从你学到的所有东西来看，你知道豆荚和在豆荚中运行的容器被认为是短暂的。这意味着它们不依赖于稳定性，因为 Kubernetes 会干预并在集群中移动它们，以符合集群中各种清单指定的期望状态。但是这里面有一个问题——我们如何处理应用中依赖于状态从一个交互持续到下一个交互的部分？如果没有某些保证，例如吊舱的可预测命名和可靠的存储操作(我们将在本章后面部分了解)，如果 Kubernetes 重新启动相关吊舱或移动它们，这种有状态组件可能会失败。然而，在深入讨论上述主题的细节之前，让我们简单地谈谈有状态应用，以及为什么在容器化环境中运行它们很有挑战性。
