...
在前一章中，我们创建了各种各样的豆荚，并管理它们的生命周期。一旦我们开始使用不同的豆荚，理想情况下，我们会希望根据某些属性对它们进行组织、分组和过滤。要做到这一点，我们需要向我们的POD添加一些信息，以便我们以后可以使用这些信息来组织它们。我们已经看到了使用`name`和`namespace`字段作为荚的元数据。除了这些字段之外，我们还可以向 pods 添加键值对，以便添加额外的信息作为标签和注释。
...
5.  Add a label using the `kubectl label` command as follows:
    ...
    输出显示`pod-without-initial-labels`POD被标记。
...
5.  Verify that all three pods are created with correct labels using the `--show-labels` argument to the `kubectl get` command. First, let's check the `frontend-production` pod:
    ...
    以下响应表明`frontend-production`POD已经创建:
...
6.  Now, check the `backend-production` pod:
    ...
    以下响应表明`backend-production`POD已经创建:
...
7.  Finally, check the `frontend-staging` pod:
    ...
    以下响应表明`frontend-staging`POD已经创建:
...
1.  Open the terminal and verify that the `frontend-production` pod we created in *Exercise 6.04*, *Selecting Pods Using Equality-Based Label Selectors*, is still running and has the required labels. We will be using the `--show-labels` argument with the `kubectl get` command:
    ...
    以下响应表明`frontend-production`POD存在:
...
2.  Verify that the `backend-production` pod we created in *Exercise 6.04*, *Selecting Pods Using Equality-Based Label Selectors* is still running and has the required labels using the `kubectl get` command with the `--show-labels` argument:
    ...
    以下响应表明`backend-production`POD存在:
...
在输出中，我们只能看到那些具有`frontend`角色的POD，而`environment`可以是任何一个给定值。因此，我们已经看到，可以根据需要混合使用不同类型的选择器。
...
注释是所有的 所以键值对可以用来存储与Kubernetes对象相关的非结构化信息。以下是 YAML 文件中注释的显示示例:
...
*   注释还可以用来添加关于客户端库或工具的信息。我们可以添加库的名称、使用的版本和公共文档链接等信息。这些信息以后可以用于我们应用中的调试问题:
...
*   注释还可以用来存储配置或检查点，这对我们应用的部署过程很有帮助。
...
考虑一下你正在支持两个团队`product-development`和`infra-libraries`。两个团队都有一些适用于不同环境(生产或试运行)的应用单元。如果情况确实如此，团队也希望将他们的POD标记为关键。

简而言之，您需要按照以下元数据要求创建三个窗格:

*   在生产环境中运行的`arbitrary-product-application`POD，归`product-development`团队所有。这需要标记为非关键 pod。
*   在生产环境中运行的`infra-libraries-application`POD，归`infra-libraries`团队所有。这需要标记为关键POD。
*   一个运行在临时环境中的`infra-libraries-application-staging`POD，归`infra-libraries`团队所有。由于它是分段运行的，所以不需要指明POD的关键程度。
...
在本章中，我们描述了标签和注释，并使用它们来添加元数据信息，这些信息可以是可用于过滤或选择对象的可识别信息，也可以是用户或工具用来获取更多关于应用状态的上下文的不可识别信息。更具体地说，我们还使用标签和注释来组织对象，例如 pods。这些都是重要的技能，可以帮助您更有效地管理 Kubernetes 对象。
