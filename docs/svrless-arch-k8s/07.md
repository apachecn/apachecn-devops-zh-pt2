...
上图类似于标准的 Kubernetes 架构，具有 Kubernetes 主节点和节点。集群中可以有一个或多个负责整体决策的 Kubernetes 主节点。Kubernetes 节点用于托管 Kubernetes 豆荚。这些POD包含软件开发人员编写的功能。控制器将使用**配置图**将功能的源代码注入POD。

这些POD将由**无库控制器**管理。在无库框架安装过程中，它将启动一个集群内控制器，持续监视功能资源。当一个功能被部署时，这个控制器将使用提供的运行时创建相关的服务、部署和 pods。
...
在下一章中，我们将介绍 OpenWhisk，并介绍 OpenWhisk动作和触发器。