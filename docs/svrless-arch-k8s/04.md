...
作为供应商锁定的解决方案，对于数据和服务的潜在重用，了解如何在 Kubernetes 上运行无服务器架构至关重要。在这一章中，介绍了Kubernetes的起源和设计。接下来，我们将安装一个本地 Kubernetes 集群，您将能够通过使用仪表板或客户端工具(如**Kubernetes**)来访问该集群。除此之外，我们将讨论 Kubernetes 应用的构建块，最后，我们将向集群部署一个真实的应用。
...
Kubernetes 集群由一个或多个服务器组成，每个服务器都被分配了一组逻辑角色。有两个基本角色分配给集群的服务器:**主节点**和**节点**。如果服务器处于**主**角色，Kubernetes的**控制平面**组件在这些节点上运行。控制平面组件是用于运行 Kubernetes API 的主要服务集，包括 REST 操作、身份验证、授权、调度和云操作。使用最新版本的 Kubernetes，有四个服务作为控制平面运行:
...
**pod** 是 Kubernetes 中计算的构件资源。pod 由计划作为单个应用运行到同一节点的容器组成。同一 pod 中的容器共享相同的资源，例如网络和内存资源。此外，pod 中的容器共享生命周期事件，例如向上或向下扩展。可以使用 **ubuntu** 映像和**回声**命令来定义POD，如下所示:
...
在 Kubernetes 中，多个应用在同一个集群中运行并相互连接。由于每个应用都有运行在不同节点上的多个 pod，因此连接应用并不简单。在 Kubernetes 中，**服务**是用于定义一组豆荚的资源，您可以使用**服务**的名称来访问它们。服务资源是使用POD的标签定义的。
...
创建**回显**作业时，Kubernetes 将创建一个 pod，对其进行调度并运行。当容器在**回显**命令后终止时，Kubernetes不会尝试重新启动它或保持它运行。
...
3.  Check the pods with the following command:
    ...
    **立方得到 pods**
    ...
    如果POD状态为**待定**，请等待几分钟，直到其变为**运行**，然后继续下一步。
...
9.  Check for the pods with the following command:
    ...
    **立方得到 pods**
    ...
    如果POD状态为**待定**或**运行**，请等待几分钟，直到变为**完成**，然后继续下一步。
