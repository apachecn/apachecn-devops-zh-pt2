...
**pod** 是 Kubernetes 中计算的构件资源。pod 由计划作为单个应用运行到同一节点的容器组成。同一 pod 中的容器共享相同的资源，例如网络和内存资源。此外，pod 中的容器共享生命周期事件，例如向上或向下扩展。可以使用 **ubuntu** 映像和**回声**命令来定义吊舱，如下所示:
...
在 Kubernetes 中，多个应用在同一个集群中运行并相互连接。由于每个应用都有运行在不同节点上的多个 pod，因此连接应用并不简单。在 Kubernetes 中，**服务**是用于定义一组豆荚的资源，您可以使用**服务**的名称来访问它们。服务资源是使用吊舱的标签定义的。
