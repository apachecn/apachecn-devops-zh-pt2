...
在上一章中，我们了解了 OpenWhisk，一个开源的无服务器框架，它是 Apache 软件基金会的一部分。我们学习了如何创建、列出、调用、更新和删除 OpenWhisk动作。我们还讨论了如何使用提要、触发器和规则来自动化操作调用。
...
当 OpenFaaS 部署在 Kubernetes 上时，Kubernetes 框架中的水平POD自动缩放功能也可以用于根据需求自动缩放功能，作为 OpenFaaS 框架中内置自动缩放功能的替代。
