...
Kubernetes 集群中的工作单位是豆荚。在Kubernetes你不能打破一个豆荚。整个POD将始终部署在一起，并接受相同的生命周期处理。pod 应该仍然是集群联盟的工作单元吗？也许能够将一个更大的单元(例如整个复制集、部署或服务)与特定的集群相关联更有意义。如果集群出现故障，整个复制集、部署或服务将被调度到不同的集群。紧密耦合的复制集集合怎么样？这些问题的答案并不总是容易的，甚至可能随着系统的发展而动态变化。
...
严格的耦合要求适用于POD必须在同一集群中的应用。如果对 pods 进行分区，应用将会失败(可能是由于跨集群联网时无法满足实时要求)，或者成本太高(pods 访问大量本地数据)。将如此紧密耦合的应用移动到另一个集群的唯一方法是在另一个集群上启动一个完整的副本(包括数据)，然后关闭当前集群上的应用。如果数据太大，应用实际上可能无法移动并且对灾难性故障敏感。这是最难处理的情况，如果可能的话，您应该设计您的系统来避免严格耦合的需求。
