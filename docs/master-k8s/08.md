...
`minReplicas`和`maxReplicas`值指定缩放范围。这是必要的，以避免失控的情况，可能会因为一些问题发生。想象一下，由于某种 bug，无论实际负载如何，每个 pod 都会立即使用 100%的 CPU。没有了 `maxReplicas`的限制，Kubernetes会不断创造越来越多的吊舱，直到所有集群资源耗尽。
...
一个库比马克星团使用Kubernetes的力量。要设置 Kubemark 集群，请执行以下步骤:
