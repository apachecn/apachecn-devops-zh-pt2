...
与 AKS 和 ACI 不同，使用 EKS 和法盖特模型模拟你的Kubernetes集群和法盖特之间的相互作用。而在 AKS 上，单个无限虚拟节点代表 ACI 的全部容量，在 EKS，每个 pod 都有自己的虚拟节点。但当然，那些节点不是真正的节点。Fargate 有自己的控制平面和数据平面，支持 EC2 和 ECS，以及 EKS。EKS-法盖特的集成是通过一组定制的Kubernetes控制器来完成的，这些控制器监视需要部署到特定命名空间或具有特定标签的吊舱，并将这些吊舱转发给法盖特安排的 T2。下图说明了 EKS 和法盖特的整合:
