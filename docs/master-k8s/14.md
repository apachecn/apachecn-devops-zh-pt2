...
最后，但并非最不重要的一点，伊斯特奥([https://istio.io/](https://istio.io/))是Kubernetes最著名的服务网格。它建立在特使之上，允许你通过 YAML 清单以库本内斯本地方式配置它。Istio 由谷歌、IBM 和 Lyft(特使开发者)发起。这是谷歌 GKE 上的一键安装，但在Kubernetes社区中被广泛使用。它也是 Knative 的默认入口/应用编程接口网关解决方案，这进一步促进了它的采用。
...
当我们回顾库本内斯的服务网格时，我们简要讨论了特使。在这里，它作为 Istio 的数据平面。特使用 C++实现，是一个高性能代理。对于服务网格中的每个POD，Istio(自动或通过 istioctl CLI)注入一个特使侧容器，以完成所有繁重的提升工作，例如:
