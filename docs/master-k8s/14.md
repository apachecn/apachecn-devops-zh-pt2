...
服务网格不接触您的应用。它将一个侧车代理容器注入每个POD和一个服务网格控制器。代理拦截POD之间的所有通信，并与网格控制器协作，处理所有交叉问题:
...
**特使**([https://www.envoyproxy.io/](https://www.envoyproxy.io/))又是一个 CNCF 毕业的项目。这是一个非常通用的和高性能的 L7 代理。它提供了许多服务网格功能，但是它被认为是非常低级和难以配置的。它也不是Kubernetes特有的。一些 Kubernetes 服务网格使用特使作为底层数据平面，并提供一个 Kubernetes 本地控制平面来配置和交互。如果您想在 Kubernetes 上直接使用特使，那么建议使用另一个开源项目，如大使或 Gloo 作为入口控制器和/或应用编程接口网关。
...
最后，但并非最不重要的一点，伊斯特奥([https://istio.io/](https://istio.io/))是Kubernetes最著名的服务网格。它建立在特使之上，允许你通过 YAML 清单以Kubernetes本地方式配置它。Istio 由谷歌、IBM 和 Lyft(特使开发者)发起。这是谷歌 GKE 上的一键安装，但在Kubernetes社区中被广泛使用。它也是 Knative 的默认入口/应用编程接口网关解决方案，这进一步促进了它的采用。
...
当我们回顾Kubernetes的服务网格时，我们简要讨论了特使。在这里，它作为 Istio 的数据平面。特使用 C++实现，是一个高性能代理。对于服务网格中的每个POD，Istio(自动或通过 istioctl CLI)注入一个特使侧容器，以完成所有繁重的提升工作，例如:
...
我们将使用迷你Kubernetes集群来检查 Istio。在安装 Istio 之前，我们应该确保我们的集群有足够的容量来处理 Istio 及其演示应用 **Bookinfo** 。我们将用 16 MB 内存和四个 CPU 启动 minikube，这应该足够了:
...
在继续之前，我们仍然需要等待所有的POD初始化，然后 Istio 将注入它的 sidecar 代理容器。当尘埃落定，你应该看到这样的东西:
