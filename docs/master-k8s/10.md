...
同样，rkt 的应用程序容器类似于 Kubernetes pods，因为它们可能包含多个 Linux 容器。如果有疑问，请记住，CNI 容器必须有自己的 IP 地址。运行库的工作是配置网络，然后执行一个或多个 CNI 插件，以 JSON 格式向它们传递网络配置。
