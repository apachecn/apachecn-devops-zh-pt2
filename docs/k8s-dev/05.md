...
豆荚不是唯一利用和暴露条件的Kubernetes资源。部署还使用条件来表示细节，例如代码更新的部署进度以及部署的整体可用性。
...
*   **准备好**:POD已经按照规格装载并启动。在就绪探测和活动探测成功完成之前(如果定义了其中一个或两个)，该值不会被标记为 true。
...
最基本的探测器是 Liveness 探测器。如果定义了，它会提供一个命令或 URL，Kubernetes 可以使用它来确定 Pod 是否仍在运行。如果调用成功，Kubernetes 将假设容器是健康的；如果它没有响应，那么可以按照`restartPolicy`的定义来处理 Pod。结果是二元的:要么探测器成功了，Kubernetes认为你的POD在运行，要么它失败了，所以Kubernetes认为你的POD不再起作用了。在后一种情况下，它将检查定义的 RestartPolicy 以选择要做什么。
