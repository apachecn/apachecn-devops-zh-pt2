...
在这一章中，你会发现处理应用级和集群级维护的方法。我们涵盖了故障排除的各个方面，从调试POD和容器，到测试服务连接，解释资源状态，以及节点维护。最后但同样重要的是，我们来看看如何处理`etcd`，Kubernetes 控制平面存储组件。本章与集群管理员和应用开发人员都相关。
...
你有一个定义明确的服务(见[食谱 5.1](05.html#simple_service) )由几个豆荚支持。 但是其中一个POD行为不检，您希望将其从端点列表中删除，以便稍后检查。

## 解决办法

使用`--overwrite`选项重新标记容器-这将允许您更改容器上`run`标签的值。通过覆盖此标签，您可以确保它不会被服务选择器([食谱 5.1](05.html#simple_service) )选择，并将从端点列表中删除。与此同时，监视你的POD的副本集将看到一个POD已经消失，并将开始一个新的副本。
...
请注意，您现在也可以使用`curl`通过`localhost`访问整个Kubernetes应用编程接口。

# 12.4 理解和解析资源状态

## 问题

您希望基于脚本或另一个自动化环境(如CI/CD流水线)中的资源状态(如 pod)做出反应。
...
你会遇到这样的情况，一个POD要么没有按预期启动，要么在一段时间后出现故障。
...
让我们看一个具体的例子，一个POD发生故障。用以下内容创建一个名为*的清单:*
...
*   [调试POD和复制控制器](https://kubernetes.io/docs/tasks/debug-application-cluster/debug-pod-replication-controller/)
...
*   Kubernetes架构设计文件中的“Kubernetes节点”

*   [主节点通信](https://kubernetes.io/docs/concepts/architecture/master-node-communication/)

*   [静态POD](https://kubernetes.io/docs/tasks/administer-cluster/static-pod/)
...
`kubectl drain`命令的作用是首先将指定的节点标记为不可调度，以防止新的POD到达(本质上是一个`kubectl cordon`)。然后，如果应用编程接口服务器支持[驱逐](http://kubernetes.io/docs/admin/disruptions/)，它将驱逐豆荚。否则，它将使用正常的`kubectl delete`删除豆荚。Kubernetes 文档有一个简洁的步骤顺序图，复制在[图 12-3](#drain-node) 中。

![Node drain sequence diagram](img/k8sc_1203.png)

###### 图 12-3。节点漏极序列图

`kubectl drain`命令驱逐或删除除镜像豆荚(不能通过 API 服务器删除)之外的所有豆荚。对于由`DaemonSet`监管的POD，`drain`在不使用`--ignore-daemonsets`的情况下不会继续，无论如何也不会删除任何`DaemonSet`管理的POD——这些POD将立即被`DaemonSet`控制器取代，该控制器会忽略不可切割的标记。
