...
首先，在一个名为*的文件中定义作业的Kubernetes清单:*
...
你想运行一个应用，要求它的POD有明显的，潜在的不同的特征，比如一个数据库，其中你有一个处理读写的领导者和几个只提供读取服务的追随者。你不能使用部署，因为他们只监督相同的豆荚，你需要一个能处理更像宠物而不是牛的豆荚的主管。
...
现在，您可以在Kubernetes仪表板中看到与POD一起创建的`StatefulSet`对象([图 7-1](#statefulset-screenshot) )。
...
最初，现在在Kubernetes被称为`StatefulSet`的东西被称为`PetSet`。这应该会让你知道动机是什么。在 Kubernetes 1.7 中，`StatefulSet`成为了一个 beta 特性，这意味着 API 不会再改变了；预计只会出现 UX 式的修复。一个`StatefulSet`是一个控制器，为它所监管的豆荚提供独特的身份。请注意，作为安全措施，删除`StatefulSet`不会删除与之相关的卷。
...
<sup xmlns:epub="http://www.idpf.org/2007/ops">[2](#idm139735593475776-marker)T3【Kubernetes小强示例在 GitHub 上，[T5【小强-statefleset . YAML](https://github.com/kubernetes/kubernetes/blob/master/examples/cockroachdb/cockroachdb-statefulset.yaml)。</sup>