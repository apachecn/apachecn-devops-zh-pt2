...
你想运行一个应用，要求它的吊舱有明显的，潜在的不同的特征，比如一个数据库，其中你有一个处理读写的领导者和几个只提供读取服务的追随者。你不能使用部署，因为他们只监督相同的豆荚，你需要一个能处理更像宠物而不是牛的豆荚的主管。
...
现在，您可以在Kubernetes仪表板中看到与吊舱一起创建的`StatefulSet`对象([图 7-1](#statefulset-screenshot) )。
