...
现在，您可以像使用任何其他卷一样使用配置映射。以下是名为`oreilly`的吊舱的清单文件；它用的是`busybox`映像，刚睡了 3600 秒。在`volumes`部分，有一个名为`oreilly`的卷，它使用了我们刚刚创建的配置图`configfile`。该体积随后被安装在容器内的路径`/oreilly`处。因此，该文件可以在 pod 中访问:
