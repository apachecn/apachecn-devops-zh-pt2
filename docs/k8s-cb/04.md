...
一个`Pod`是一个`/api/v1`对象，和任何其他Kubernetes对象一样，它的清单文件包含以下字段:
...
*   设置应启动和监督的POD(`replicas`)或相同副本的数量。
...
当你想要摆脱一个部署，以及它所管理的副本集和POD时，执行一个类似`kubectl delete deploy/fancyapp`的命令。不要*试图删除单个POD，因为它们将由部署重新创建。这是经常让初学者困惑的事情。*
