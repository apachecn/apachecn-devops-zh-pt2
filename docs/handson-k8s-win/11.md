...
最后一章演示了如何在 Kubernetes 中使用容器化的 Windows 应用——现在，我们将扩展我们的投票应用，以使用更高级的功能，使编排更加健壮和自动化。多年来，Kubernetes 扩展了越来越多的功能，从细粒度的**基于角色的访问控制**(**【RBAC】**)或机密管理到使用**水平 POD 自动缩放器** ( **HPA** )的自动缩放，后者是容器编排的圣杯。当然，我们无法在本书的范围内涵盖所有这些功能，但我们将包括有助于运行容器化 Windows 应用的最有用的功能。此外，请记住，当您运行内部 Kubernetes 集群时，某些功能不可用，例如，特定于云的存储资源调配器—我们将要介绍的所有示例都假设您运行的是 AKS Engine Kubernetes 集群。
...
*   如果您想要编辑应用的源代码并进行调试，请使用 Microsoft Visual Studio 2019 社区(或任何其他版本)—Visual Studio Code 对经典版本的支持有限。NET 框架
*   蔚蓝账户
*   使用 AKS 引擎部署 Windows/Linux Kubernetes 集群，准备部署上一章中的投票应用
...
在上一章中，我们已经使用了一个命名空间(名为`dev`)来将我们应用的组件逻辑分组到一个现有的物理 Kubernetes 集群中的一个虚拟集群中。命名空间的一般原则是为对象名称提供资源配额和范围，给定命名空间内的名称必须是唯一的，但它们不必在不同的命名空间中是唯一的。默认情况下，Kubernetes 开箱即用地提供以下命名空间:

*   `kube-system`:Kubernetes 系统创建的对象的命名空间，如`kube-apiserver`或`kube-proxy` Pods。
...
在前面的几章中，您已经多次使用了这个构造。但是，最好知道，如果没有为命令提供名称空间，它将使用在当前 kubeconfig 上下文中设置为默认值的名称空间。换句话说，它不必是默认的名称空间——这完全取决于您的上下文设置。我们已经在[第 6 章](06.html)、*与 Kubernetes 集群*交互中深入介绍了上下文—为了完整起见，我们将展示如何更改当前上下文中使用的命名空间。要在当前上下文中永久设置`prod`名称空间，请使用以下命令:
...
*   **准备状态探测器**:用于确定给定的容器是否准备好接受运输。只有当一个 POD 的所有容器都准备好了，它才被认为是准备好了。未就绪的 pod 将从服务端点中删除，直到它们再次就绪。
...
*   只要容器启动后，容器可能还没有准备好为交通服务，就使用这个探测器。
...
*   对于公开 HTTP 端点的微服务应用，请考虑始终配置`httpGet`就绪探测器。这将确保当容器成功运行但 HTTP 服务器未完全初始化时，覆盖所有情况。
*   在您的应用中使用单独的、专用的 HTTP 端点进行就绪性检查是一个好主意，例如，一个常见的约定是使用`/health`。
*   如果在这种类型的探测中检查依赖项(外部数据库和日志记录服务)的状态，请小心共享依赖项，例如投票应用中的 SQL Server。在这种情况下，您应该考虑使用探测超时，它大于外部依赖关系允许的最大超时，否则，您可能会出现级联故障和较低的可用性，而不是偶尔增加延迟。

对于使用 **IIS** (简称**互联网信息服务**)托管的 web 应用来说，准备情况调查很有意义—IIS 应用池需要完全启动，数据库迁移可能尚未应用。例如，我们将为投票应用配置一个简单的就绪探测器，如下所示:

*   ASP.NET MVC 应用将实现一个服务于`/health`请求的专用控制器。
...
2.  此外，您需要记住在默认路由映射之前，在`RouteConfig`类中修改您的应用的路由配置。
...
3.  与前一章一样，构建应用的 Docker 映像，将其标记为 1.1.0 版本，并将其推送到 Docker Hub。在我们的演示案例中，我们将使用`packtpubkubernetesonwindows/voting-application:1.1.0`映像。
...
7.  在网络浏览器中，当您导航到应用时，您应该不会遇到任何 IIS 应用池启动延迟—网络服务器将通过就绪检查进行预热。
...
*   应谨慎使用活性探针。此探测器的错误配置会导致服务中的级联故障和容器重启循环。作为一个快速的实验，您可以重新部署投票应用清单，用活动探测器替换就绪探测器，配置相似，但超时和延迟非常短，您将经历多次随机崩溃和应用可用性差！
*   除非有充分的理由，否则不要使用活性探测器。例如，一个很好的原因可能是您的应用中存在已知的死锁问题，其根本原因尚不清楚。
...
如果你不清楚`ServiceMonitor.exe`和`LogMonitor.exe`入口点进程的幕后到底发生了什么，那么由 IIS 托管的 Web 应用可能是使用活跃度探测器的一个很好的选择。理论上，每当 IIS 或 IIS 应用池出现问题时，他们应该会崩溃容器，但是让我们假设我们需要自己实现这些检查。我们将实现一个活跃度探测器，它将使用`exec`处理程序检查 IIS 应用池是否正在运行。为此，请遵循以下步骤:
...
探测器被配置为执行 PowerShell 命令`if ((Get-WebAppPoolState DefaultAppPool).Value -ne "Started") { throw "Default IIS App Pool is NOT started" }`，该命令检查默认的 IIS 应用池是否处于`Started`状态。如果不是，将引发异常，PowerShell 进程将以非零退出代码退出，导致探测器进入失败状态。
...
我们的投票应用不需要专用的启动探测器，但是部署清单文件中的示例定义可能如下所示:
...
在下一节中，我们将重点关注为 Pods 分配资源限制，以及如何为我们的投票应用配置自动缩放。
...
当您创建一个 Pod 时，可以指定它的容器需要多少计算资源——在上一章中，我们已经执行了一个为投票应用分配资源的简短练习。一般来说，计算资源是 CPU 和 RAM 内存—Kubernetes 还能够管理其他资源，例如 Linux 上的大型内存或本地节点上的临时存储。
...
*   `requests`:指定系统提供给定资源的保有量。您也可以反过来考虑这个问题——这是 Pod 容器要求系统正常运行的给定资源量。POD 调度取决于`requests`值(而不是`limits`)。
...
我们在[第 4 章](04.html)、*Kubernetes 概念和窗口支持*中介绍了 Kubernetes 窗口节点上资源管理支持的细节。重要的一点是，Windows 目前缺乏对内存不足杀手的支持(对内存限制的一些支持可能会在 Kubernetes 中的传入 Hyper-V 容器功能中提供)。这意味着超过为 Windows 容器内存设置的`limits`值不会导致任何节流或容器重启。这里，经验法则是使用`requests`仔细管理内存调度，并监控任何突然的内存分页。
...
对于内存，我们遵循当前针对 Windows 节点的建议—我们只指定希望请求多少内存。为了让 CPU 模拟资源耗尽，我们指定了一个大的请求值，该值将消耗 Windows 节点的所有集群 CPU。这样做的原因是，两个带有 Azure 虚拟机类型 Standard_D2_v3 的节点各有两个虚拟桌面，并且运行五个副本，我们总共需要五个虚拟桌面。更新`strategy`需要删除，以避免在部署期间出现任何死锁。

2.  使用`kubectl apply -f .\voting-application.yaml`命令应用清单文件。
3.  现在，仔细观察部署中新 Pods 的创建。你会注意到有显示`Pending`状态的 POD:
...
现在，您已经知道如何为容器分配和管理资源，我们可以演示如何使用 HPA 为您的应用使用自动缩放。
...
我们的投票应用公开了不需要太多 CPU 的功能，这意味着可能很难按需触发自动缩放。为了解决这个问题，我们将添加一个专用的控制器动作，它可以用给定的目标百分比值模拟恒定的 CPU 负载。用于压力模拟的`packtpubkubernetesonwindows/voting-application:1.2.0` Docker 映像的源代码可以在[https://github . com/PacktPublishing/hand-Kubernetes-On-Windows/tree/master/chapter 11/08 _ voting-application-HPA-src](https://github.com/PacktPublishing/Hands-On-Kubernetes-on-Windows/tree/master/Chapter11/08_voting-application-hpa-src)上找到。如果您想自己自定义应用，请在 Visual Studio 2019 中打开您的解决方案，并按照以下步骤操作:
...
4.  构建带有标签`1.2.0`的 Docker 映像，并将其推送到您的存储库中，就像我们之前所做的那样。

映像准备就绪后，我们可以继续部署新版本的投票应用并配置自动缩放。为此，请执行以下步骤:

1.  修改`voting-application.yaml`清单文件，确保您使用了映像的`1.2.0`标签，并且`resources`的指定如下:
...
当 IIS 应用池完全初始化时，每个 Pod 的 CPU 使用率应稳定在`150m`左右。
...
祝贺您，您已经成功地为投票应用配置了 HPA。我们将要演示的下一个 Kubernetes 特性是使用配置映射和机密来注入配置数据。

# 使用配置映射和机密管理应用配置

为在 Kubernetes 上运行的应用提供配置，有两种可能的方法，记录在[https://kubernetes.io/docs/tasks/inject-data-application/](https://kubernetes.io/docs/tasks/inject-data-application/)中:
...
首先，让我们来看看《秘密》。几乎在每个应用中，您都必须管理用于访问依赖项的敏感信息，例如密码、OAuth 令牌或证书。由于明显的安全问题和非常有限的灵活性，将这些信息作为硬编码值放在 Docker 映像中是不可能的。同样，不建议直接在 Pod 清单文件中定义密码—清单文件旨在保持在源代码控制中，这绝对不是存储此类敏感信息的地方。为了管理这类信息，Kubernetes 提供了 Secret 对象，从技术上讲，它可以保存任何类型的由键值对组成的数据。或者，可以在`etcd`中加密静态机密，这在生产场景中是推荐的。

我们现在将演示如何使用`kubectl`创建通用(不透明)秘密。您也可以为此目的使用清单文件，但是如何生成这些清单文件取决于您的 CI/CD 流水线(您不想将这些清单文件签入源代码管理！).要为 SQL Server 密码创建密码，请执行以下步骤:
...
3.  现在，秘密可以作为装载在容器中的卷(作为文件或目录)使用，或者用于定义容器的环境变量。在投票应用的情况下，更容易使用带有 SQL Server 密码的机密作为环境变量。这是通过部署清单中的以下方式实现的:
...
这里的关键概念是使用`secretKeyRef`从我们刚刚创建的`mssql`秘密中引用`SA_PASSWORD`密钥的值。该值被注入到`MSSQL_SA_PASSWORD`环境变量中(但是您可以检查在使用`kubectl describe`时不可能看到该值！)，容器中运行的应用可以访问它。在我们的例子中，我们使用这个变量来定义另一个名为`CONNECTIONSTRING_VotingApplication`的环境变量。例如，当您需要创建一个必须包含密码的连接字符串时，这是一种常见的模式，但请记住，这可能是一种不如使用卷安全的解决方案。

将机密用作环境变量和装载卷之间有一个显著的区别:如果机密发生变化，通过卷提供的机密数据将被更新。根据您的需求和实施细节，您可能希望选择将机密装载为卷。当然，这需要您的应用知道机密文件的可能更改，这意味着它需要主动监控文件系统并刷新任何凭据提供程序、连接字符串或证书，这些通常保存在内存中。将机密视为不可变的配置值是最好的选择(无论是作为卷还是作为环境变量装载时)，并且使您的应用更具可预测性，更不复杂。但是，如果您的体系结构有限制，希望尽可能少的 Pod 重启，那么将 Secrets 作为一个卷注入并在您的应用中实现自动刷新是建议的解决方案。
...
为了存储应用的非敏感配置数据，Kubernetes 提供了 ConfigMap 对象。这是您可以用来从运行时配置数据中完全分离 Docker 映像(您的构建工件)的另一个概念。从应用编程接口的角度来看，这个概念类似于机密——您可以存储键值对，并将它们作为容器的环境变量注入，或者使用卷作为文件或目录来装载它们。为了演示这一点，我们将创建一个配置图，用于存储 ASP.NET MVC 应用的`Web.config`文件中引用的配置文件`customErrors.config`，并使用卷装载它。
...
1.  首先，我们需要对投票应用源代码进行一个小的修改。我们将把`<customErrors>`节点从`<system.web>`节点提取到子目录中的一个单独文件中。在`Web.config`文件中，将`<system.web>`节点更改为:
...
6.  修改用于部署的`voting-application.yaml`清单文件，将我们的配置图装载为容器中的目录(记住使用新的 Docker 映像标签):
...
11.  再次导航至`http://<serviceExternalIp>/Home/StressCpu`。如果您的 IIS 应用池已重新加载，您将看到完整的异常详细信息，而不是自定义错误页面。

通过这种方式，我们演示了如何在 Windows Pods 中使用机密和配置映射。现在，是时候熟悉管理 Windows 节点上的持久数据存储了。

# 管理 Windows 节点上的持久数据存储

在[第 4 章](04.html)、 *Kubernetes 概念和 Windows 支持*中，我们已经介绍了 Kubernetes 中与存储相关的一些概念，例如**persistent volumes**(**PV**)、**persistent volume claims**(**PVC**)和**storagelces**(**SC**)以及它们在 Windows 工作负载中是如何得到支持的。管理容器化应用中的状态和存储以及使用状态集是一个广泛而复杂的话题，不在本书的讨论范围内——官方文档提供了一个很好的介绍，可以在[https://kubernetes.io/docs/concepts/storage/](https://kubernetes.io/docs/concepts/storage/)找到。Windows Pods 的 PersistentVolume 支持的关键是，您可以使用一些现有的卷插件，但不是全部。在 Windows 上，支持以下功能:
...
我们的投票应用已经对运行在 Linux Pod 中的 SQL Server 使用了 PersistentVolumes 在本例中，我们已经对`kubernetes.io/azure-disk`置备程序使用了 StorageClass，它在内部使用了 azureDisk 卷插件。这个场景与 Linux Pods 有关——现在，我们将为 Windows Pods 使用 PersistentVolumes。投票应用不需要在前端容器中保存数据，但是作为一个纯粹的例子，我们将展示如何为每个 Pod 存储投票日志。
...
1.  为投票应用构建一个带有标签`1.4.0`的 Docker 映像，并将其推送到 Docker Hub，就像前面的例子一样。
...
状态集合要求提供负责该状态集合的服务名称(`1`)。除此之外，我们定义了`volumeClaimTemplates` ( `4`)，它将用于为这个状态集中的每个 Pod 副本创建一个专用的 PersistentVolumeClaim。我们将这个用于安装卷的聚氯乙烯作为容器(`3`)中的`C:/data`目录，其中`voting.log`将被持久化。此外，我们还需要向 IIS 应用池用户授予对`C:/data`目录的适当读/写权限，否则，网络应用将无法访问我们的持久卷。这是使用在`init`容器(`2`)中执行的`icasls.exe`来实现的。请注意，在分配权限之前，您需要首先启动 IIS ( `iisreset.exe /START`)以正确创建 IIS 应用池用户！

6.  使用`kubectl apply -f .\voting-application.yaml`命令应用清单文件。
7.  当状态集准备好之后，在网络浏览器中导航到该应用并投票几次。
...
2.  等待直到所有 POD 终止，并使用`kubectl get pods -n dev`命令进行观察。
...
4.  等待 POD 创建并准备就绪。由于我们的准备就绪探测器，可能需要几分钟。
5.  在网络浏览器中导航至`http://<serviceExternalIp>/Home/VotingLog`。您应该会看到与之前完全相同的投票日志。这表明所有 Pods 都装载了与以前相同的持久卷。

恭喜你！您已经在投票应用的窗口窗格中成功装载了 azureDisk PersistentVolumes。接下来，我们将了解如何为您的应用配置滚动更新。

# 为部署配置滚动更新

在生产场景中，您肯定需要一种部署策略，为您的应用提供零停机更新。作为一个容器编排者，Kubernetes 提供了不同的构建模块，可用于实现蓝绿色部署、加那利部署或滚动部署。Kubernetes Deployment 对象完全支持执行滚动更新部署—在这种类型的部署中，应用的新版本是通过逐渐用新副本替换旧副本来推出的，所有这些副本都在同一服务的后面。这意味着，在部署过程中，最终用户将达到应用的旧版本或新版本。
...
让我们看看如何为投票应用实现滚动部署。事实上，我们已经在前面的示例中使用了这种方法，现在我们将更详细地解释配置。请遵循以下步骤:
...
4.  让我们看看它在实践中是如何工作的。首先，使用`kubectl apply -f .\voting-application.yaml`命令应用清单文件——这将创建应用的初始版本。
5.  现有部署的展开可以通过实时编辑对象或使用`kubectl rollout`命令来强制完成。一般来说，最好使用声明性方法:更改清单文件并再次应用它。在清单文件中将容器映像标签更改为`packtpubkubernetesonwindows/voting-application:1.4.0`，并使用`kubectl apply -f .\voting-application.yaml`命令应用。
...
8.  您可以尝试在部署期间访问该应用。我们已经正确配置了就绪探测器，因此您不会遇到来自应用的任何意外响应！
...
*   使用最小特权原则。您的应用应该只能访问自己的资源(建议您使用专用服务帐户运行每个应用，该帐户可以访问该应用的机密或配置映射)。根据用户在项目中的角色，他们应该有受限的访问权限(例如，质量保证工程师可能对集群只有只读访问权限)。
*   将`RoleBinding`分配给组，而不是单个用户。这将使您的权限管理更加容易。请注意，这需要与外部身份验证提供程序集成才能发挥最佳功能。

让我们演示如何使用投票应用的`Role`和`RoleBinding`将对部署的访问限制在所需的最少一组配置映射和机密。我们将在 ASP.NET MVC 应用中这样做，在 SQL Server 中使用类似的方法可能是一个额外的练习。为此，我们将使用投票应用 Docker 映像，`packtpubkubernetesonwindows/voting-application:1.3.0`，我们用它来演示配置地图。此部署在运行时需要配置映射和机密。请按照以下步骤配置 RBAC:
...
3.  为`Role`创建`role.yaml`清单文件，用于读取应用的机密和配置映射:
...
在本章中，我们展示了 Kubernetes 的几个常用的高级特性。首先，您了解了 Kubernetes 中名称空间的用途以及如何管理它们。然后，我们介绍了就绪性、活性和启动探测器，它们用于监控 Pod 容器的生命周期，并且我们为您提供了一组使用探测器时的推荐做法以及如何避免常见陷阱。下一步是学习如何指定 Pod 资源请求和限制，以及如何使用 HPA 将其与自动缩放相结合。为了将配置数据(包括敏感密码)注入我们的应用，我们使用了配置映射和机密。除此之外，我们还演示了如何在运行于 Windows 节点上的状态集中使用 PersistentVolumes(由 azureDisk Volume 插件支持)。最后，您学习了如何处理部署对象的滚动更新，以及 RBAC 在 Kubernetes 的目的是什么。

下一章将重点介绍使用 Kubernetes 的开发工作流，以及在创建 Kubernetes 应用时如何与其他开发人员合作。
...
*   有关 Kubernetes 功能和如何管理应用的更多信息，请参考以下 Packt 书籍:
