...
*   `kube-system`:Kubernetes系统创建的对象的命名空间，如`kube-apiserver`或`kube-proxy` Pods。
...
在前面的几章中，您已经多次使用了这个构造。但是，最好知道，如果没有为命令提供名称空间，它将使用在当前 kubeconfig 上下文中设置为默认值的名称空间。换句话说，它不必是默认的名称空间——这完全取决于您的上下文设置。我们已经在[第 6 章](06.html)、*与Kubernetes集群*交互中深入介绍了上下文—为了完整起见，我们将展示如何更改当前上下文中使用的命名空间。要在当前上下文中永久设置`prod`名称空间，请使用以下命令:
...
*   **准备状态探测器**:用于确定给定的容器是否准备好接受运输。只有当一个POD的所有容器都准备好了，它才被认为是准备好了。未就绪的 pod 将从服务端点中删除，直到它们再次就绪。
...
*   `requests`:指定系统提供给定资源的保有量。您也可以反过来考虑这个问题——这是 Pod 容器要求系统正常运行的给定资源量。POD调度取决于`requests`值(而不是`limits`)。
...
我们在[第 4 章](04.html)、*Kubernetes概念和窗口支持*中介绍了Kubernetes窗口节点上资源管理支持的细节。重要的一点是，Windows 目前缺乏对内存不足杀手的支持(对内存限制的一些支持可能会在 Kubernetes 中的传入 Hyper-V 容器功能中提供)。这意味着超过为 Windows 容器内存设置的`limits`值不会导致任何节流或容器重启。这里，经验法则是使用`requests`仔细管理内存调度，并监控任何突然的内存分页。
...
3.  现在，仔细观察部署中新 Pods 的创建。你会注意到有显示`Pending`状态的POD:
...
我们现在将演示如何使用`kubectl`创建通用(不透明)秘密。您也可以为此目的使用清单文件，但是如何生成这些清单文件取决于您的CI/CD流水线(您不想将这些清单文件签入源代码管理！).要为 SQL Server 密码创建密码，请执行以下步骤:
...
2.  等待直到所有POD终止，并使用`kubectl get pods -n dev`命令进行观察。
...
4.  等待POD创建并准备就绪。由于我们的准备就绪探测器，可能需要几分钟。
