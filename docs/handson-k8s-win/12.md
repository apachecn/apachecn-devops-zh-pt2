...
还可以为现有的 ASP.NET 核心增加Kubernetes/赫尔姆支持:
...
*   确保应用得到适当的升级，包括依赖性管理。
*   根据您的需求配置软件部署。掌舵图基本上是通用的**另一种标记语言** ( **YAML** )Kubernetes清单的参数化模板。Helm 使用 Go 模板([https://godoc.org/text/template](https://godoc.org/text/template))进行参数化。如果你熟悉围棋，那么你就在家；如果没有，那么你会发现它与其他模板系统非常相似，比如小胡子。
