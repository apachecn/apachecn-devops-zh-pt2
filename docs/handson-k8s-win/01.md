...
3.  通过选择使用Windows容器而不是 Linux 容器选项，默认情况下启用Windows容器支持:
...
现在，让我们用示例网页创建一个进程隔离的Windows容器。在 Visual Studio Code 中，导航到命令面板(*Ctrl*+*Shift*+*P*)并找到 Docker: Run 命令。作为映像，选择`docker-helloworld-iis`。带有适当命令的终端将会打开。
